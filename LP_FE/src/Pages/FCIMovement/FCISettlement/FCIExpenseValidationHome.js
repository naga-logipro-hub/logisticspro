...
const FCIExpenseValidationHome = () => {
  /*================== User Location Fetch ======================*/
  ...
  const exportToCSV = () => {
    ...
    let fileName='FCIExpenseValidation_'+dateTimeString
  ...
  const [fciClosureApprovaldata, setFciClosureApprovaldata] = useState([])
  ...
  const getClosureVehiclesData = () => {
    ...
    FCIClosureSubmissionService.getExpenseValidationData().then((res) => {
      expenseClosureData = res.data.data
      console.log(expenseClosureData,'getExpenseValidationData')

      setFciClosureApprovaldata(expenseClosureData)
      setFetch(true)
      let rowDataList = []
      ...
      expenseClosureData.map((data, index) => {
        ...
        rowDataList.push({
          ...
          Vendor_Type: data.expense_vendor_type == 1 ? 'Freight' : 'Loading',
          ...
          Screen_Duration: data.expense_approval_at_time,
          Action: (
            <CButton className="badge" color="warning">
              <Link className="text-white" to={`/FCIExpenseValidation/${data.closure_id}`}>
                {/* Expense Approval  */}
                Expense Validation
              </Link>
            </CButton>
...
export default FCIExpenseValidationHome
