...
const FCIIncomeClosureHome = () => {
  ...
  const exportToCSV = () => {
    ...
    let fileName='FCIIncomeClosure_'+dateTimeString
  ...
  const getClosureVehiclesData = () => {
    ...
    FCIClosureSubmissionService.getIncomeClosureData().then((res) => {
      expenseClosureData = res.data.data
      console.log(expenseClosureData,'getIncomeClosureData')
      ...
      expenseClosureData.map((data, index) => {
        ...
        rowDataList.push({
          ...
          Screen_Duration: data.expense_posting_at_time,
          Action: (
            <CButton className="badge" color="warning">
              <Link className="text-white" to={`/FCIIncomeClosure/${data.closure_id}`}>
                 Income Posting   
              </Link>
            </CButton>
          ),
...
export default FCIIncomeClosureHome
