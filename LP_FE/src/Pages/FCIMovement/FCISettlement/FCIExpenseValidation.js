...
const FCIExpenseValidation = () => {
  ...
  const exportToCSV = () => {
    ...
    let fileName='FCI_MIGO_Expense_In_Expense_Validation_'+dateTimeString
  ...
  let page_no = LogisticsProScreenNumberConstants.FCIClosureModule.FCI_Expense_Validation
  ...
  const rejectAllExpenseSubmission = () => {
    ...
    formDataUpdate.append('status', 5) /* 5 - Expense Validation Rejected */
    formDataUpdate.append('remarks', values.expens_remarks)
    console.log(id,'id')
    console.log(values.inclremarks,'expens_remarks')
    ...
    for (var i = 0; i < vendorPoGroupingMigoData.length; i++) {
      SentVAInfoData.VA_NO = vendorPoGroupingMigoData[i].pp_va_no
      SentVAInfoData.TRIPSHEET_NO = vendorPoGroupingMigoData[i].tripsheet_no
      SentVAInfoData.VENDOR_TYPE = vendorPoGroupingMigoData[i].vendor_type ? (vendorPoGroupingMigoData[i].vendor_type == 2 ? 3 : vendorPoGroupingMigoData[i].vendor_type) : 2  
    ...
    FCITripsheetSapService.sentVAInfoToSAP(SentVAInfoData_seq).then((res) => {
      ...
      if(sap_tsc_res && sap_tsc_res.length > 0){ 
        ...
        if(singleClosureSeqArray.expense_vendor_type == 2){
          FCIClosureSubmissionService.rejectLoadingExpenseSubmission(formDataUpdate).then((res) => { 
            console.log(res)
            setFetch(true)
            if (res.status == 200) {
              Swal.fire({
                title: "Expense Validation Rejected Successfully!",
                icon: "success",
                confirmButtonText: "OK",
              }).then(function () {
                navigation('/FCIExpenseValidationHome')
              });
              ...
              toast.warning(
                'Expense Validation Rejection Cannot Be Updated From LP.. Kindly Contact Admin!'
              )
            }
          })
          .catch((error) => {
          ...
          FCIClosureSubmissionService.rejectFreightExpenseSubmission(formDataUpdate).then((res) => { 
            console.log(res)
            setFetch(true)
            if (res.status == 200) {
              Swal.fire({
  ...
  useEffect(() => {
    ...
    FCIClosureSubmissionService.getTripClosureInfoById(id).then((res) => { 
        ...
        if(closure_info_id_data.status == 7){
          ...
          values.tdsType = closure_info_id_data.expense_tds_type ? closure_info_id_data.expense_tds_type : ''
  ...
  const FCIExpenseValidationReject = () => {
    console.log(values.expens_remarks,'inclremarks')
    if (values.expens_remarks && values.expens_remarks.trim()) {
      setTripMigoAllDelete(true)
    } else {
      setFetch(true)
      Swal.fire({
        title: 'Trip Total Expense Remarks required for rejection..',
        icon: "warning",
        confirmButtonText: "OK",
      }).then(function () {
      })
  ...
  const FCITripsheetExpenseValidationSubmit = (type) => {
    ...
    if(user_info.is_admin == 1) {
      console.log(singleClosureSeqArray,'RTECS-singleClosureSeqArray')
      ...
      console.log(invoicePostingDate,'RTECS-invoicePostingDate')
      ...
      console.log(values.gstType,'RTECS-gstType')
      console.log(tdsHave,'RTECS-tdsHave')
      console.log(values.tdsType,'RTECS-tdsType')
    }
    // return false
    ...
    formDataUpdate.append('expense_tds_type', tdsHave == '1' ? values.tdsType : '')
    ...
    formDataUpdate.append('status', 4) // 4 - Expense Validation Verified
    ...
    FCIClosureSubmissionService.submitExpenseValidation(formDataUpdate).then((res) => {
      setFetch(true)
      console.log(res,'submitExpenseValidation')
      if (res.status == 200) {
        Swal.fire({
        ...
        toast.warning(
          'Expense Validation Submission Cannot Be Updated From LP.. Kindly Contact Admin!'
        )
      }
    })
    .catch((error) => {
            ...
            <>
              <CCard className="p-1">
                        ...
                        <CTable caption="top" hover style={{ height: '35vh' }}>
                          ...
                          <CTableBody>
                            ...
                            <CTableRow>
                              ...
                              <CTableDataCell>Total Trip Expenses <REQ /></CTableDataCell>
                        ...
                        <CRow>
                          ...
                          >
                            ...
                            <CButton
                              ...
                              className="mx-3 text-white" 
                              onClick={() => { 
                                FCITripsheetExpenseValidationSubmit(expenseUpdateAvailable ? 'update' : 'submit')
                              }}
                              type="submit"
                            >
                ...
                >
                  ...
                  <CModalBody>
                    <p className="lead">Are you sure to reject this Expense Validation ?</p>
                    </CModalBody>
                  <CModalFooter>
                    <CButton
                      className="m-2"
...
export default FCIExpenseValidation
