...
const FCIExpensePostingHome = () => {
  ...
  const exportToCSV = () => {
    ...
    let fileName='FCIExpensePosting_'+dateTimeString
  ...
  const getClosureVehiclesData = () => {
    ...
    FCIClosureSubmissionService.getExpenseValidationApprovalData().then((res) => {
      expenseClosureData = res.data.data
      console.log(expenseClosureData,'getExpenseValidationApprovalData')
        ...
        rowDataList.push({
          ...
          Action: (
            <CButton className="badge" color="warning">
              <Link className="text-white" to={`/FCIExpensePosting/${data.closure_id}`}>
                 Expense Posting   
              </Link>
            </CButton>
          ),
...
export default FCIExpensePostingHome
