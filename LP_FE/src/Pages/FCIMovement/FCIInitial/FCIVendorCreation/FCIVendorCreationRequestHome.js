...
const FCIVendorCreationRequestHome = () => {
  /*================== User Location Fetch ======================*/
  ...
  const exportToCSV = () => {
    ...
    let fileName='FCI_vendor_creation_request_screen_'+dateTimeString
  ...
  function cancelVRC(id) {
    setFetch(false)
    FCIVendorCreationService.cancelVendorInfo(id).then((res) => {
      setFetch(true)
      toast.success('Vendor Request Cancelled Successfully!')
      setTimeout(() => {
        window.location.reload(false)
      }, 1000)
    })
  ...
  const loadVendorCreationTable = () => {
    FCIVendorCreationService.getVendorRequestTableData().then((res) => {
      setFetch(true)
        ...
        rowDataList.push({
          ...
          Action: (
            <div className="d-flex justify-content-space-between">
              ...
              <Link to={`FCIVendorCreationRequest/${data.vendor_id}`}>
                <CButton
                  size="sm" 
                  color="secondary"
                  shape="rounded"
          ...
          <CCard className="mt-4">
            <CContainer className="mt-2">
              <CRow>
                ...
                >
                  <Link className="text-white" to="/FCIVendorCreationNewRequest">
                    <CButton size="sm" color="warning" className="mx-3 px-3 text-white" type="button">
                      NEW
                    </CButton>
                  </Link>
                  <CButton
...
export default FCIVendorCreationRequestHome
