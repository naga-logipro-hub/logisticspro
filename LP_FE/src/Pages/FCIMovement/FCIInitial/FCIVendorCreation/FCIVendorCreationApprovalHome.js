...
const FCIVendorCreationApprovalHome = () => {
  /*================== User Location Fetch ======================*/
  ...
  const exportToCSV = () => {
    ...
    let fileName='FCI_vendor_creation_approval_screen_'+dateTimeString
  ...
  const loadVendorCreationTable = () => {
    FCIVendorCreationService.getVendorRequestApproveTableData().then((res) => {
      setFetch(true)
        ...
        rowDataList.push({
          ...
          Waiting_At: (
            <span className="badge rounded-pill bg-info">
              {"VC Request Created ✔️"}
            </span>
          ),
          Action: (
            <div className="d-flex justify-content-space-between">
             
                <Link to={`VendorCreationApproval/${data.vendor_id}`}>
                <CButton
                    size="sm" 
                    color="secondary"
                    shape="rounded"
...
export default FCIVendorCreationApprovalHome
