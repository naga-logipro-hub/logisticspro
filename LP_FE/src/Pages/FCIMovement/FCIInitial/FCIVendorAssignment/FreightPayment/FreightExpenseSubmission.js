...
const FreightExpenseSubmission = () => {
  ...
  let page_no = LogisticsProScreenNumberConstants.FCIModule.FCI_FP_Vendor_Assignment
  ...
  const setAdditionalCharges = (event, child_property_name, parent_index) => {
    ...
    if(child_property_name == 'atti_cooli_having'){ 
  ...
  const setAttiCooliValueHaving = (original, input) => {
    // return input === undefined ? original : input
    return input === undefined ? original : input
  }
  ...
  const total_trip_freight = (data) => {
    ...
    let toll_charges = data.toll_freight_input ? Number(data.toll_freight_input)
    : 0
    let loading_charges = data.loading_freight_input ? Number(data.loading_freight_input)
    : 0
    let updated_freight_charges = data.changed_freight_input ? Number(data.changed_freight_input)
    : 0

    let sap_overall_expense = data.OVERALL_EXPENSE

    let atti_cooli_havingValue = data.atti_cooli_having_input && data.atti_cooli_having_input == 1 ? data.ATTI_COOLI : 0

    let budget_freight_amount = updated_freight_charges != 0 ? updated_freight_charges : 0

    let fCharge = atti_cooli_havingValue + budget_freight_amount + toll_charges + loading_charges + expense_additional_charges - expense_deduction_charges

    let total_data1 = data.STATUS == 2 ?  fCharge + sap_overall_expense : fCharge /* 2 - Same Vendor, 1 - Different Vendor */
    ...
    console.log(loading_charges, 'loading_charges')
  ...
  const exportToCSV = () => {
    ...
    let fileName='FCIFreightExpenseSubmissionMigoDetails_'+dateTimeString
...
useEffect(() => {
  ...
  if(popGroupingData.length > 0){
    ...
    FCITripsheetSapService.getpopVendorDeliveryData(p__no,p__code).then((res) => {
      setFetch(true)
    ...
    FCIClosureSubmissionService.getPopVendorWiseTripFreightMigoData(p__no,p__code).then((res) => {
      let lp_response1 = res.data.data
      console.log(lp_response1, 'getPopVendorWiseTripFreightMigoData')
...
const checkMigoDataFinder = (tsno1,migono1,type) => {
  ...
  vendorPopGroupingLPMigoData.map((vv1,kk1)=>{
    if(tsno1 == vv1.tripsheet_no && migono1 == vv1.migo_no){
      if(type == 1) {
        ...
        needed_data = vv1.nlld_updated_freight
      } else if(type == 2) {
        ...
        needed_data = vv1.toll_freight
      } else if(type == 7) {
        needed_data = vv1.atti_cooli_having
      } else if(type == 8) {
        needed_data = vv1.loading_freight
      }
    }
  })
  ...
  },[values.expremarks,vendorHaving])
  ...
  function checkModalDisplay() {
    ...
    /* Remarks Mandatary Part for Chaged Freight condition */
    // if(change_remarks == 1){
  ...
  const migoBudgetExpenseFinder = (dat) => {
    ...
    dat.map((vv,kk)=>{
      ...
      budget_exp_migo += Number(parseFloat(vv.changed_freight_input).toFixed(3)) 
    })
  ...
  const migoBudgetWMTExpenseFinder = (dat) => {
    let budget_WMT_exp_migo = 0
    console.log(dat,'migoBudgetWMTExpenseFinder')

    dat.map((vv,kk)=>{
      budget_WMT_exp_migo += vv.FREIGHT_CHARG 
    })
    console.log(budget_WMT_exp_migo,'budget_WMT_exp_migo')
    return Number(parseFloat(budget_WMT_exp_migo).toFixed(3))
  }
  ...
  const vIDFinder = (vcode) => {
    let vID = '--'
    fciVendorsData.map((datann1, indexnn1) => {
  ...
  const expenseSubmit = () => {
    ...
    fci_closure_data.append('nlfd_base_expenses', migoBudgetWMTExpenseFinder(migoData))
    fci_closure_data.append('nlld_base_expenses', migoBudgetExpenseFinder(migoData))
    fci_closure_data.append('other_expenses', additionalExpenseFinder(migoData))
    ...
    fci_closure_data.append('expense_vendor_type', 1)
    ...
    for (var i = 0; i < migoData.length; i++) {
      ...
      SentVAInfoData.VENDOR_TYPE = migoData[i].STATUS == 2 ? 3 : 1   /* 1 - Freight Vendor, 2 - Loading Vendor, 3 - Freight & Loading Vendor */      
      ...
      if(sap_tsc_res && sap_tsc_res.length > 0){
        ...
        FCIClosureSubmissionService.createClosure(fci_closure_data).then((res) => {
          ...
          if (res.status == 200) {
            Swal.fire({
              icon: "success",
              title:'Freight Expense Submission Request Sent Successfully!',
              text:  'Freight Expense Reference : ' + expense_reference,
              confirmButtonText: "OK",
            }).then(function () {
              if(popGroupingData[0].tripsheet_count == migoData.length){
                navigation('/FreightExpenseSubmissionHome')
              } else {
                window.location.reload(false)
              }
            });
                    ...
                    <CRow style={{justifyContent:"center"}}>
                      ...
                      {/* <CCol xs>
                        <CInputGroup>
                          ...
                          <CFormInput readOnly value={popGroupingData[0].vendor_code} />
                        </CInputGroup>
                      </CCol> */}
                    </CRow>
                                ...
                                <CTableRow key={`trip_expense_data${index}`}>
                                    ...
                                    {checkMigoStatus(data.TRIPSHEET_NO,data.MIGO_NO) ? (
                                      ...
                                      <CFormInput
                                        ...
                                        value={setTripExpense(
                                          data.changed_freight,
                                          data.changed_freight_input
                                        )}
                                        size="sm"
                                      />
                                    }
                                    ...
                                    {checkMigoStatus(data.TRIPSHEET_NO,data.MIGO_NO) ? (
                                      <>
                                        <CFormInput
                                          ...
                                          value={checkMigoDataFinder(data.TRIPSHEET_NO,data.MIGO_NO,5)}
                                          readOnly
                                        />
                                      </> ) :
                                      <CFormInput
                                    ...
                                    {checkMigoStatus(data.TRIPSHEET_NO,data.MIGO_NO) ? (
                                      <>
                                        <CFormInput
                                          ...
                                          name="toll_freight"
                                          id="toll_freight"
                                          value={checkMigoDataFinder(data.TRIPSHEET_NO,data.MIGO_NO,6)}
                                          readOnly
                                        />
                                      </> ) :
                                      <CFormInput
                                      ...
                                      <>
                                        {checkMigoStatus(data.TRIPSHEET_NO,data.MIGO_NO) ? (
                                          <CFormInput
                                            ...
                                            id="loading_freight"
                                            value={checkMigoDataFinder(data.TRIPSHEET_NO,data.MIGO_NO,8)}
                                            readOnly
                                          />
                                        ) : (
                                          <CFormInput
                                  ...
                                  >
                                    ...
                                    {checkMigoACHStatus(data.TRIPSHEET_NO,data.MIGO_NO,data.STATUS) ? (
                                          <CFormCheck
                                            size="lg"
                                            name="atti_cooli_having"
                                            id="atti_cooli_having"  
                    ...
                    <CRow className="mt-md-3">
                      <CCol className="" xs={12} sm={12} md={3}>
                        <CButton size="sm" color="primary" className="text-white" type="button">
                          <Link className="text-white" to="/FreightExpenseSubmissionHome">
                            Previous
                          </Link>
                        </CButton>
                      </CCol>
...
export default FreightExpenseSubmission
