...
const LoadingExpenseSubmissionApprovalHome = () => {
  /*================== User Location Fetch ======================*/
  ...
  let page_no = LogisticsProScreenNumberConstants.FCIModule.FCI_LP_Vendor_Assignment_APPROVAL
  ...
  const exportToCSV = () => {
    ...
    let fileName='FCILoadingPaymentVendorAssignmentApproval_'+dateTimeString
  ...
  const getClosureVehiclesData = () => {
    ...
    FCIClosureSubmissionService.getLoadingExpenseApprovalData().then((res) => {
      closureApprovalData = res.data.data
      console.log(closureApprovalData[0],'getLoadingExpenseApprovalData')
        ...
        rowDataList.push({
          ...
          Action: (
            <CButton className="badge" color="warning">
              <Link className="text-white" to={`/LoadingExpenseSubmissionApproval/${data.closure_id}`}>
                Expense Approval
                {/* Income Closure */}
              </Link>
            </CButton>
...
export default LoadingExpenseSubmissionApprovalHome
