...
import {
  ...
  const FCIPlantMasterEdit = () => {
    ...
    const [singlePlantData, setSinglePlantData] = useState(false) 
    ...
    useEffect(() => {
        ...
        //section to fetch single Plant Master info
        FCIPlantMasterService.getFCIPlantListById(id).then((response) => {
            setFetch(true)
            let resData = response.data.data
            console.log(response,'getFCIPlantListById')
            values.plant_name = resData.plant_name
            values.plant_symbol = resData.plant_symbol
            values.company_name = resData.company_name
            values.street_no = resData.street_no
            values.street_name = resData.street_name
            values.city = resData.city
            values.state = resData.state
            values.postal_code = resData.postal_code
            values.region = resData.region
            values.gst_no = resData.gst_no
            values.remarks = resData.remarks
            setSinglePlantData(resData)
        })
    ...
    function updatePlant() {
        ...
        if(values.plant_name != singlePlantData.plant_name && locationNameExists(values.plant_name)){
            toast.warning('Plant Name already exists..')
            return false
        }
      
        if(values.plant_symbol != singlePlantData.plant_symbol && locationSymbolExists(values.plant_symbol)){
        ...
        formData.append('plant_code', singlePlantData.plant_code) 
        formData.append('created_by', singlePlantData.created_by) 
        ...
        FCIPlantMasterService.updateFCIPlantListById(id, formData).then((res) => {
                        ...
                        <CRow className="mb-md-1">
                          ...
                          <CCol md={3}>
                              ...
                              <CFormInput
                                  ...
                                  readOnly 
                                  type="text"
                                  value={singlePlantData.plant_code} 
                              />
                          </CCol>
                          <CCol md={3}>
  ...
  export default FCIPlantMasterEdit
