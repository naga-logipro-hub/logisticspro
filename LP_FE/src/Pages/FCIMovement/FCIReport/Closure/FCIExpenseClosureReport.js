...
const FCIExpenseClosureReport = () => {
  ...
  const onChangeFilter = (event, event_type) => {
    ...
    } else if (event_type == 'closure_status') {
      if (selected_value) {
        setReportClosureStatus(selected_value)
      } else {
        setReportClosureStatus('')
      }
  ...
  const exportToCSV = () => {
    ...
    let fileName='FCI_Expense_Report_'+dateTimeString
  ...
  const [reportClosureStatus, setReportClosureStatus] = useState('')
  ...
  const loadTripSheetExpenseReport = (fresh_type = '') => {
    ...
    if (fresh_type !== '1') {
      ...
      FCIClosureSubmissionService.getTripsheetExpenseDataForReport().then((res) => { 
          ...
          rowDataList.push({
            ...
            Action: (
              <CButton className="badge" color="warning">
                <Link className="text-white" to={`/FCIExpenseReport/${data.closure_id}`}>
                  View
                </Link>
              </CButton>
            ),
      ...
      report_form_data.append('status', reportClosureStatus) 
      ...
      console.log('status', reportClosureStatus)
      console.log('fci_plant_id', reportPlantId)

      // setFetch(true)
      // return false

      FCIClosureSubmissionService.sentTripsheetExpenseDataForReport(report_form_data).then((res) => { 
        console.log(res, 'res')
        tableReportData = res.data.data
            ...
            <CCard className="mt-4">
              ...
              <CCol xs={12} md={3}>
                ...
                <FCIVendorSerachSelectComponent
                  ...
                  className="mb-2"
                  onChange={(e) => {
                    onChangeFilter(e, 'closure_status')
                  }}
                  label="Select Expense Status"
                  noOptionsMessage="Status Not found"
                  search_type="fci_expense_report_status"
                  search_data={searchFilterData}
                />
              </CCol>
            </CRow>
...
export default FCIExpenseClosureReport
