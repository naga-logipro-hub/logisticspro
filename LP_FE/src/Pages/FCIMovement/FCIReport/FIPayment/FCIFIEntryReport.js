...
const FCIFIEntryReport = () => {
  ...
  let page_no = LogisticsProScreenNumberConstants.FCIReportModule.FCI_FIEntry_report
  ...
  const exportToCSV = () => {
    ...
    let fileName='Fci_FIEntry_Report_'+dateTimeString
  ...
  const rakeFIEntryReport = (fresh_type = '') => {
    ...
    if (fresh_type !== '1') {
      ...
      FCIClosureSubmissionService.getFiEntryDataForReport().then((res) => {
        setFetch(true)
        tableReportData = res.data.data
        setSearchFilterData(tableReportData)
        console.log(res.data.data,'getFCIFiEntryDataForReport')
        let rowDataList = []
        ...
        filterData.map((data, index) => {
          ...
          if(data.fi_entry_mode == 3 || data.fi_entry_mode == 4){
            ...
            rowDataList.push({
              ...
              RPS_NO: data.rps_no,
              FNR_NO: data.fnr_no,
              RAKE_LOC: data.fci_closure_info?.fci_plant_code,
              RAKE_LOC: locationFinder(data.fci_closure_info?.fci_plant_code),
              RPS_DATE: data.created_at_date,
              VENDOR_NAME: data.fci_closure_info?.expense_vendor_name,
...
export default FCIFIEntryReport
