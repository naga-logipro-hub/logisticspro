...
const tsvchahapp = () => { 
  ...
  /* ==================== Access Part End ========================*/
    ...
    const exportToCSV = (e1) => {
        ...
        let fileName='VCH_AH_Approval_Report_'+dateTimeString
    ...
    const vchStatusFinder = (data, type = 0) => {
        ...
        if(type == 1){
            if(data.vch_time_info && data.vch_time_info.vch_requested_at){
                let temp = timeDiffFormatted(data.vch_time_info.vch_requested_at)
                return temp
            }
        }
        if(data.vch_status == 2){
    ...
    const loadTripShipmentReport = (fresh_type = '') => {
        ...
        if (fresh_type !== '1') {   
            VendorCreationService.getTSVendorChangeAhApprovalRequestData().then((res) => { 
                console.log(res,'res')  
                ...
                rowDataList.push({
                    ...
                    Action: (
                        <CButton className="badge" color="warning">
                            <Link className="text-white" target='_blank' to={`AHApproval/${goToEncryptedProfile(data.trip_sheet_id)}`}> 
                                VCH AH Approval
                            </Link>
                        </CButton>
                    ), 
