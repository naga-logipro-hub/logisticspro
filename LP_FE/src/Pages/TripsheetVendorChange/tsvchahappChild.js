...
const tsvchahappChild = () => {
  ...
  let page_no = LogisticsProScreenNumberConstants.TSVendorChangeModule.TSVCH_AH_Approval
  ...
  const VCRAHApproval = (status) => {

    toast.success(`Vendor Change Request - AH Approval ${status == 1 ? 'Confirmation': 'Rejection'} Process Validated..!`)
    // setFetch(true)
    // return false
    ...
    formData.append('vch_ah_approval_remarks', values.remarks || '')
    formData.append('vch_ah_approved_by', user_id)
    formData.append('vch_status', status == 1 ? 3 : 1) /* 1 - Request Rejected , 3 - AH Approved */
    
    if(status == 2){
      formData.append('rejection_type', 1) /* AH Rejection */
    }
      ...
      .then((res) => {
        ...
        if (res.status == 200) {
            Swal.fire({
                ...
                text: `Vendor Change Request has been ${status == 1 ? "Approved." : "Rejected."}.`,
                icon: "success"
            }).then(function () {
                navigation('/tsvchahapp')
            })
...
export default tsvchahappChild
