/* tsvchreq : Trip Sheet Vendor Change Request Screen */
...
const tsvchreq = () => { 
  ...
  /* ==================== Access Part End ========================*/
    ...
    const exportToCSV = (e1) => {
        ...
        let fileName='VCH_Request_Report_'+dateTimeString
    ...
    const vchStatusFinder = (data) => {
        let status = '-'

        if(data.trip_sheet_info && data.trip_sheet_info.vch_status == 1){
            status = 'Rejected'
        } else if(data.trip_sheet_info && data.trip_sheet_info.vch_status == 4){
    ...
    const loadTripShipmentReport = (fresh_type = '') => {
        ...
        if (fresh_type !== '1') {   
            VendorCreationService.getTSVendorChangeRequestData().then((res) => { 
                console.log(res,'res')  
                ...
                let filterData = tableReportData.filter(
                    (data) => !(data.trip_sheet_info.vch_status == 2 || data.trip_sheet_info.vch_status == 3)
                )
                ...
                rowDataList.push({
                    ...
                    PAN_Number : data.vendor_info ? data.vendor_info.pan_card_number : '-', 
                    Vch_Status : vchStatusFinder(data),
                    Action: (
                        <CButton className="badge" color="warning">
                        <Link className="text-white" target='_blank' to={`VendorChangeRequest/${goToEncryptedProfile(data.tripsheet_sheet_id)}`}>
                        {/* <Link className="text-white" to={`VendorChangeRequest/${data.parking_yard_gate_id}`}> */}
                            VCH Request
                        </Link>
                        </CButton>
                    ), 
