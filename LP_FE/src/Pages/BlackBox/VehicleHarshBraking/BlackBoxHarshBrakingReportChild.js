...
const BlackBoxHarshBrakingReportChild = () => {
  ...
  let page_no = LogisticsProScreenNumberConstants.BlackBoxVehicleReportModule.Vehicle_Harsh_Braking
  ...
  const assignVehicleInfo = (vdata,vno) => {
    ...
    let needed_data = filterData[0].HarshBrakingSubLst
  ...
  useEffect(() => {
    BlackBoxService.getVHBInfoById(id).then((res) => {
      setFetch(true)
      let rowDataList = []
      ...
      vehicle_info_data.map((data, index) => { 
        rowDataList.push({
          ...
          over_speed_count: data.harsh_braking_count, 
          Action: (
            <>
                <CButton 
                    className="btn btn-sm me-md-1" 
                    color={data.harsh_braking_count == 0 ? 'danger' : 'success'}
                    disabled={data.harsh_braking_count==0}
                    onClick={() => {
                        setAssignMigoModal(true)
                        setDisplayVehicleNo(data.vehicle_no)
                        setDisplayVehicleOSCount(data.harsh_braking_count)
                        setDisplayVehicleInfo(data.vehicle_info)
                        assignVehicleInfo(closure_info_data.json_data.aaData,data.vehicle_no)
            ...
            <>
              <CCard className="p-1">
                      ...
                      <CTabPane role="tabpanel" aria-labelledby="home-tab" visible={activeKey === 1}>
                        ...
                        <CRow className="mb-md-1">
                          <CCol
                            ...
                            md={12} 
                          > 
                            <Link to={'/BlackBoxHarshBrakingReportScreen'}>
                              <CButton
                                size="s-lg"
                                color="primary"
                ...
                {/* ============== Income Reject Confirm Button Modal Area ================= */}
                  ...
                  >
                  <CModalHeader>
                      <CModalTitle>{`Vehicle No : ${displayVehicleNo}, Harse Braking Count : ${displayVehicleOSCount}`}</CModalTitle>
                  </CModalHeader>
                  <CModalBody>
...
export default BlackBoxHarshBrakingReportChild
