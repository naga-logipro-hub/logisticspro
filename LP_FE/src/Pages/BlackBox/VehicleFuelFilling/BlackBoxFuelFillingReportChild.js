...
const BlackBoxFuelFillingReportChild = () => {
  ...
  let page_no = LogisticsProScreenNumberConstants.BlackBoxVehicleReportModule.Vehicle_Fuel_Filling
  ...
  const assignVehicleInfo = (vdata,vno) => {
    ...
    let needed_data = filterData[0].FuelFillingMainModel  
    ...
    needed_data.map((data, index) => { 
      rowDataList.push({
        ...
        first_filling: data.FirstFilling,
        first_filling_time: data.StartDateTime,
        last_filling: data.LastFiliing,
        last_filling_time: data.EndateTime,
        total_fuel: data.DifferenceFuelLevel,
        distance_covered: data.Distance,
  ...
  const columns_child = [
    ...
    {
      name: 'First Filling',
      selector: (row) => row.first_filling,
      sortable: true,
      center: true,
    },
    {
      name: 'Time',
      selector: (row) => row.first_filling_time,
      sortable: true,
      center: true,
    },
    {
      name: 'Last Filling',
      selector: (row) => row.last_filling,
      sortable: true,
      center: true,
    },
    {
      name: 'Time',
      selector: (row) => row.last_filling_time,
      sortable: true,
      center: true,
    },
    {
      name: 'Total Fuel',
      selector: (row) => row.total_fuel,
      sortable: true,
      center: true,
    },
    {
      name: 'Distance Covered',
      selector: (row) => row.distance_covered,
      sortable: true,
      center: true,
    },
    // {
  ...
  useEffect(() => {
    BlackBoxService.getVFFInfoById(id).then((res) => {
      setFetch(true)
      let rowDataList = []
      ...
      vehicle_info_data.map((data, index) => { 
        rowDataList.push({
          ...
          fuel_filling_count: data.fuel_filling_count, 
          Action: (
            <>
              <CButton 
                className="btn btn-sm me-md-1" 
                color={data.fuel_filling_count == 0 ? 'danger' : 'success'}
                disabled={data.fuel_filling_count==0}
                onClick={() => {
                    setAssignMigoModal(true)
                    setDisplayVehicleNo(data.vehicle_no)
                    setDisplayVehicleOSCount(data.fuel_filling_count)
  ...
  const columns = [
    ...
    {
      name: 'Fuel Filling Count',
      selector: (row) => row.fuel_filling_count,
      sortable: true,
      center: true,
    }, 
    {
          ...
          {screenAccess ? (
            <>
              <CCard className="p-1">
                      ...
                      <CTabPane role="tabpanel" aria-labelledby="home-tab" visible={activeKey === 1}>
                        ...
                        <CRow className="mb-md-1">
                          <CCol
                            ...
                            md={12} 
                          > 
                            <Link to={'/BlackBoxFuelFillingReportScreen'}>
                              <CButton
                                size="s-lg"
                                color="primary"
                ...
                >
                <CModalHeader>
                    <CModalTitle>{`Vehicle No : ${displayVehicleNo}, Fuel Filling Count : ${displayVehicleOSCount}`}</CModalTitle>
                </CModalHeader>
                <CModalBody>
...
export default BlackBoxFuelFillingReportChild
