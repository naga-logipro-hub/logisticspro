...
const BlackBoxFuelFillingReportScreen = () => {
  ...
  const exportToCSV = () => {
    ...
    let fileName='BB_Vehicle_Rash_Turn_Report_'+dateTimeString
  ...
  const loadVehicleOverSpeedReport = (fresh_type = '') => {

    if (fresh_type !== '1') {
      // console.log(user_locations)
      /*================== User Location Fetch ======================*/

      BlackBoxService.getVehilceFuelFillingInfo().then((res) => {
  
        tableReportData = res.data.data  
        console.log(tableReportData,'getVehilceFuelFillingInfo')
        setFetch(true)
        let rowDataList = []
        ...
        filterData.map((data, index) => { 
          rowDataList.push({
            ...
            Vehicle_Count: data.display_record_count,
            // Remarks: data.remarks ? data.remarks : '-',
            Creation_Time: data.created_date,
            Action: (
              <>
                {data.display_record_count != 0 ? (
                  <CButton className="btn btn-success btn-sm me-md-1">
                    <Link className="text-white" target='_blank' to={`/BlackBoxFuelFillingReport/${data.id}`}>
      ...
      BlackBoxService.sentVehilceFuelFillingInfo(report_form_data).then((res) => { 

        tableReportData = res.data.data  
        console.log(tableReportData,'sentVehilceFuelFillingInfo')
...
export default BlackBoxFuelFillingReportScreen
