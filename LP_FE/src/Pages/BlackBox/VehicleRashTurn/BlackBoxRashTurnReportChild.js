...
const BlackBoxRashTurnReportChild = () => {
  ...
  let page_no = LogisticsProScreenNumberConstants.BlackBoxVehicleReportModule.Vehicle_Rash_Turn
  ...
  const assignVehicleInfo = (vdata,vno) => {
      ...
      let needed_data = filterData[0].RashTurnSubLst
  ...
  useEffect(() => {
      ...
      BlackBoxService.getVRTInfoById(id).then((res) => {
        setFetch(true)
        let rowDataList = []
        ...
        vehicle_info_data.map((data, index) => { 
          rowDataList.push({
            ...
            over_speed_count: data.rash_turn_count, 
            Action: (
              <>
                  <CButton 
                      className="btn btn-sm me-md-1" 
                      color={data.rash_turn_count == 0 ? 'danger' : 'success'}
                      disabled={data.rash_turn_count==0}
                      onClick={() => {
                          setAssignMigoModal(true)
                          setDisplayVehicleNo(data.vehicle_no)
                          setDisplayVehicleOSCount(data.rash_turn_count)
            ...
            <>
              <CCard className="p-1">
                      ...
                      <CTabPane role="tabpanel" aria-labelledby="home-tab" visible={activeKey === 1}>
                        ...
                        <CRow className="mb-md-1">
                          <CCol
                            ...
                            md={12} 
                          > 
                            <Link to={'/BlackBoxRashTurnReportScreen'}>
                              <CButton
                                size="s-lg"
                                color="primary"
                ...
                {/* ============== Income Reject Confirm Button Modal Area ================= */}
                  ...
                  >
                  <CModalHeader>
                      <CModalTitle>{`Vehicle No : ${displayVehicleNo}, Rash Turn Count : ${displayVehicleOSCount}`}</CModalTitle>
                  </CModalHeader>
                  <CModalBody>
...
export default BlackBoxRashTurnReportChild
