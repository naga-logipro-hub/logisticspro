...
const BlackBoxFuelTheftReportChild = () => {
  ...
  let page_no = LogisticsProScreenNumberConstants.BlackBoxVehicleReportModule.Vehicle_Fuel_Theft
  ...
  const assignVehicleInfo = (vdata,vno) => {
    ...
    let needed_data = filterData[0].FueltheftMainModel  
  ...
  useEffect(() => {
    BlackBoxService.getVFTInfoById(id).then((res) => {
      setFetch(true)
      let rowDataList = []
      ...
      vehicle_info_data.map((data, index) => { 
        rowDataList.push({
          ...
          over_speed_count: data.fuel_theft_count, 
          Action: (
            <>
                <CButton 
                    className="btn btn-sm me-md-1" 
                    color={data.fuel_theft_count == 0 ? 'danger' : 'success'}
                    disabled={data.fuel_theft_count==0}
                    onClick={() => {
                        setAssignMigoModal(true)
                        setDisplayVehicleNo(data.vehicle_no)
                        setDisplayVehicleOSCount(data.fuel_theft_count)
            ...
            <>
              <CCard className="p-1">
                ...
                {/* ============== Income Reject Confirm Button Modal Area ================= */}
                  ...
                  >
                  <CModalHeader>
                      <CModalTitle>{`Vehicle No : ${displayVehicleNo}, Fuel Theft Count : ${displayVehicleOSCount}`}</CModalTitle>
                  </CModalHeader>
                  <CModalBody>
                    ...
                    <CRow className="mb-md-1">
                      <CCol
                        ...
                        md={12} 
                      > 
                        <Link to={'/BlackBoxFuelTheftReportScreen'}>
                          <CButton
                            size="s-lg"
                            color="primary"
...
export default BlackBoxFuelTheftReportChild
