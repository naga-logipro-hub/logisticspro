...
const RakeExpenseClosureReport = () => {
  ...
  const exportToCSV = () => {
    ...
    let fileName='Rake_Expense_Report_'+dateTimeString
  ...
  const loadTripSheetExpenseReport = (fresh_type = '') => {
    ...
    if (fresh_type !== '1') {
      ...
      RakeClosureSubmissionService.getTripsheetExpenseDataForReport().then((res) => {
          ...
          rowDataList.push({
            ...
            Action: (
              <CButton className="badge" color="warning">
                <Link className="text-white" to={`/RakeExpenseReport/${data.closure_id}`}>
                  View
                </Link>
              </CButton>
            ),
      ...
      RakeClosureSubmissionService.sentTripsheetExpenseDataForReport(report_form_data).then((res) => {
...
export default RakeExpenseClosureReport
