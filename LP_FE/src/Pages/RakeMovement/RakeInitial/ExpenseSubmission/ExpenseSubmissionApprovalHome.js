...
const ExpenseSubmissionApprovalHome = () => {
  /*================== User Location Fetch ======================*/
  ...
  const exportToCSV = () => {
    ...
    let fileName='RakeExpenseApproval_'+dateTimeString
  ...
  const getClosureVehiclesData = () => {
    ...
    RakeClosureSubmissionService.getExpenseApprovalData().then((res) => {
      ...
      console.log(closureApprovalData[0],'getExpenseApprovalData')
        ...
        rowDataList.push({
          ...
          Action: (
            <CButton className="badge" color="warning">
              <Link className="text-white" to={`/ExpenseSubmissionApproval/${data.closure_id}`}>
                Expense Approval
                {/* Income Closure */}
              </Link>
            </CButton>
...
export default ExpenseSubmissionApprovalHome
