...
const RakeIncomeClosureHome = () => {
  ...
  const getClosureVehiclesData = () => {
    ...
    RakeClosureSubmissionService.getIncomeClosureData().then((res) => {
      closureApprovalData = res.data.data
      console.log(closureApprovalData,'getExpenseApprovalData')

      setRakeClosureApprovaldata(closureApprovalData)
      setFetch(true)
      let rowDataList = []
        ...
        rowDataList.push({
          ...
          Action: (
            <CButton className="badge" color="warning">
              <Link className="text-white" to={`/RakeIncomeClosure/${data.closure_id}`}>
                {/* Expense Approval  */}
                Income Closure
              </Link>
            </CButton>
...
export default RakeIncomeClosureHome
