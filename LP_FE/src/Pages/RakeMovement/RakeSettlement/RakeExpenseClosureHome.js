...
const RakeExpenseClosureHome = () => {
  ...
  const getClosureVehiclesData = () => {
    ...
    RakeClosureSubmissionService.getExpenseClosureData().then((res) => {
      expenseClosureData = res.data.data
      console.log(expenseClosureData,'getExpenseClosureData')
      ...
      expenseClosureData.map((data, index) => {
        ...
        rowDataList.push({
          ...
          Screen_Duration: data.income_closure_at_time,
          Action: (
            <CButton className="badge" color="warning">
              <Link className="text-white" to={`/RakeExpenseClosure/${data.closure_id}`}>
                {/* Expense Approval  */}
                Expense Validation
              </Link>
            </CButton>
...
export default RakeExpenseClosureHome
