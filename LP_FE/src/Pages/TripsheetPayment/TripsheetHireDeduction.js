...
const TripsheetHireDeduction = () => {
  ...
  let page_no = LogisticsProScreenNumberConstants.HireDeductionPaymentModule.Hire_Deduction_Screen
  ...
  const [deductionSubmit, setDeductionSubmit] = useState(false)
  ...
  const nlld_hire_payment_deduction_gl = '418013'
  const nlcd_hire_payment_deduction_gl = '418006'
  const nlld_hire_payment_deduction_cost_center = ['LD-UNLOAD','LD-SUB-DEL','LD-WEIGHCH','LD-FREIGHT','LD-ST-DIV','LD-HALT-CH','LD-TOLL' ]
  const nlcd_hire_payment_deduction_cost_center = ['CD-UNLOAD','CD-SUB-DEL','CD-WEIGHCH','CD-FREIGHT','CD-ST-DIV','CD-HALT-CH','CD-TOLL' ]
  ...
  const gettripHirePaymentData = (e) => {
    ...
    if(/^[a-zA-Z0-9]+$/i.test(tripsheetNumberNew) && /[a-zA-Z]/.test(tripsheetNumberNew) && /[0-9]/.test(tripsheetNumberNew)){
      TripSheetClosureService.getTripSettlementDeductionInfoByTripsheetNo(tripsheetNumberNew).then((res) => {
  ...
  const deductionUpdate = (e,type) => {
    let val = e.target.value
    const re = /^[0-9\b]+$/;

    if (re.test(e.target.value)) {
      if(type == 1){
        setUnloadingDeduction(val)
      } else if(type == 2){
        setSubdeliveryDeduction(val)
      } else if(type == 3){
        setWeighmentDeduction(val)
      } else if(type == 4){
        setFreightDeduction(val)
      } else if(type == 5){
        setDiversionReturnDeduction(val)
      } else if(type == 6){
        setHaltingDeduction(val)
      } else if(type == 7){
        setTollDeduction(val)
      }
    } else {
      if(type == 1){
        setUnloadingDeduction(0)
  ...
  const deductionSubmissionValidation = () => {

    /* ===================== Validations Part Start ===================== */

    let check_data = tripHirePaymentData.trip_settlement_info
    ...
    if(getInt(check_data.unloading_charges) < getInt(unloadingDeduction)){
      setFetch(true)
      toast.warning(`Unloading deduction should be less than additional unloading charges..`)
      return false
    }

    if(getInt(check_data.sub_delivery_charges) < getInt(subdeliveryDeduction)){
      setFetch(true)
      toast.warning(`Sub delivery deduction should be less than additional sub delivery charges..`)
      return false
    }

    if(getInt(check_data.weighment_charges) < getInt(weighmentDeduction)){
      setFetch(true)
      toast.warning(`Weighment deduction should be less than additional weighment charges..`)
      return false
    }

    // if(getInt(check_data.freight_charges) < getInt(freightDeduction)){
    //   setFetch(true)
    //   toast.warning(`Freight deduction should be less than additional freight charges..`)
    //   return false
    // }

    if(getInt(check_data.diversion_return_charges) < getInt(diversionReturnDeduction)){
      setFetch(true)
      toast.warning(`Stock Diverison / Return deduction should be less than additional Stock Diverison / Return charges..`)
      return false
    }

    if(getInt(check_data.halting_charges) < getInt(haltingDeduction)){
      setFetch(true)
      toast.warning(`Halting deduction should be less than additional halting charges..`)
      return false
    }

    if(getInt(check_data.toll_amount) < getInt(tollDeduction)){
      setFetch(true)
      toast.warning(`Toll deduction should be less than additional toll charges..`)
      return false
    }

    // if(getInt(tripHirePaymentData.trip_settlement_info.expense) < getInt(totalDeductionAmount)){
    //   setFetch(true)
    //   toast.warning(`Total deduction amount should be less than total trip expenses..`)
    //   return false
    // }

    if(totalDeductionAmount == 0){
      setFetch(true)
      toast.warning(`Deduction amount should be greater than 0`)
      return false
    }

    if(deductionRemarks == ''){
      setFetch(true)
      toast.warning(`Deduction Remarks should not be empty.`)
      return false
    }

    if(paymentPostingDate ==''){
      setFetch(true)
      toast.warning(`Please check and choose Deduction Posting date..`)
      return false
    }
    ...
    setDeductionSubmit(true)
  }

  /* Submit Process */
  const TripsheetDeductionPaymentSubmit = () => {
    ...
    console.log(tripHirePaymentData,'tripHirePaymentData==1')
    console.log(tripHirePaymentData.trip_sheet_info.to_divison,'tripHirePaymentData==trip_sheet_info.to_divison')
    console.log(tripHirePaymentData.trip_settlement_info.sap_book_division,'tripHirePaymentData==trip_settlement_info.sap_book_division')

    for (var i = 0; i < deduction_type_amount.length; i++) {

      HireDeduction.LINE_ITEM = i+1

      if(tripHirePaymentData.trip_sheet_info.to_divison == 2 && tripHirePaymentData.trip_settlement_info.sap_book_division == 2)
      {
        HireDeduction.GL_CODE = nlcd_hire_payment_deduction_gl
        HireDeduction.COST_CENTER = nlcd_hire_payment_deduction_cost_center[i]
      } else {
        HireDeduction.GL_CODE = nlld_hire_payment_deduction_gl
        HireDeduction.COST_CENTER = nlld_hire_payment_deduction_cost_center[i]
      }
      
      HireDeduction.DEDUCTION_AMT = deduction_type_amount[i]
    ...
    SAPApendData.TRIPSHEET_NO = tripHirePaymentData.trip_sheet_info.trip_sheet_no
    SAPApendData.VEHICLE_NO = tripHirePaymentData.vehicle_number
    SAPApendData.LIFNR = hireVendorCode
    SAPApendData.DEDUCT_REMARKS = deductionRemarks
    SAPApendData.DEDUCTION_TOT_AMT = totalDeductionAmount
    SAPApendData.POST_DATE = paymentPostingDate
    ...
    /* ====== Request Sent To SAP For Deduction Doc. Generation Start ========== */

    TripSheetClosureSapService.hireTripsheetDeductionPost(SAPApendData_Seq).then((res) => {

      console.log(res,'hireTripsheetDeductionPost')

      let sap_deduction_doc_no = res.data.DEDUCT_DOCUMENT_NO
      let sap_deduction_status = res.data.STATUS
      let sap_deduction_message = res.data.MESSAGE
      let sap_deduction_tripsheet = res.data.TRIPSHEET_NO

      console.log(sap_deduction_doc_no + '/' + sap_deduction_status + '/' + sap_deduction_message + '/' + sap_deduction_tripsheet)

      if(res.status == 200 && sap_deduction_status == '1' && sap_deduction_doc_no && sap_deduction_message && sap_deduction_tripsheet == tripHirePaymentData.trip_sheet_info.trip_sheet_no) {

          /* ====== Request Sent To SAP For Deduction Doc. Generation End ========== */

          /* ====== Request Sent To LP DB For Deduction Details Updation Start ========== */
          ...
          form_data.append('tripsheet_no', sap_deduction_tripsheet)
          ...
          form_data.append('total_deduction', totalDeductionAmount)
          ...
          // form_data.append('posted_balance_payment', user_id)
          form_data.append('deduction_remarks', deductionRemarks)
          // form_data.append('payment_remarks', user_id)
          form_data.append('deduction_posting_date', paymentPostingDate)
          // form_data.append('payment_posting_date', user_id)
          // form_data.append('payment_created_by', user_id)
          // form_data.append('payment_creation_time', user_id)
          form_data.append('sap_deduction_doc_no', sap_deduction_doc_no)
          // form_data.append('sap_payment_doc_no', user_id)
          // form_data.append('current_vendor_outstanding', user_id)
          ...
          TripSheetClosureService.createHireTripsheetDeduction(form_data).then((res)=>{
            ...
            if (res.status == 200) {
              Swal.fire({
                title: 'Hire Tripsheet - Deduction Completed Successfully!',
                icon: "success",
                text:  'Deduction Doc No : ' + sap_deduction_doc_no,
                confirmButtonText: "OK",
              }).then(function () {
                window.location.reload(false)
              });
              ...
              toast.warning('Hire Tripsheet Deduction Failed. Kindly contact Admin..!')
            }
          })
          .catch((error) => {
            setFetch(true)
        ...
        setFetch(true)
        toast.warning('Deduction Failed in SAP.. Kindly Contact Admin!')
      }
              ...
              <CContainer className="mt-2">
                ...
                {smallfetch && Object.keys(tripHirePaymentData).length != 0  && (
                  ...
                  <CCard style={{display: tripSheetHaving ? 'block' : 'none'}}  className="p-3">
                    ...
                    <CTabContent>
                      ...
                      <CTabPane role="tabpanel" aria-labelledby="profile-tab" visible={activeKey === 5}>
                        <CRow className="mt-2">
                          ...
                          {tripHirePaymentData.trip_settlement_info.unloading_charges != 0 && (
                            <CCol xs={12} md={3}>
                              <CFormLabel htmlFor="truckClean">
                                Unloading Deduction <REQ />{' '}
                              </CFormLabel>
                              <br />
                              <CFormInput
                                id="unloading_deduction_charges"
                                name="unloading_deduction_charges"
                                maxLength={5}
                                size="sm"
                                value={unloadingDeduction}
                                onChange={(e) => {deductionUpdate(e,'1')}}

                              />
                            </CCol>
                          )}
                          {tripHirePaymentData.trip_settlement_info.sub_delivery_charges != 0 && (
                            <CCol xs={12} md={3}>
                              <CFormLabel htmlFor="subdelivery_deduction_charges">
                                Sub Delivery Deduction <REQ />{' '}
                              </CFormLabel>
                              <br />
                              <CFormInput
                                id="subdelivery_deduction_charges"
                                name="subdelivery_deduction_charges"
                                maxLength={5}
                                size="sm"
                                value={subdeliveryDeduction}
                                onChange={(e) => {deductionUpdate(e,'2')}}
                              />
                            </CCol>
                          )}
                          {tripHirePaymentData.trip_settlement_info.weighment_charges != 0 && (
                            <CCol xs={12} md={3}>
                              <CFormLabel htmlFor="weighment_deduction_charges">
                                Weighment Deduction <REQ />{' '}
                              </CFormLabel>
                              <br />
                              <CFormInput
                                id="weighment_deduction_charges"
                                name="weighment_deduction_charges"
                                maxLength={5}
                                size="sm"
                                value={weighmentDeduction}
                                onChange={(e) => {deductionUpdate(e,'3')}}
                              />
                            </CCol>
                          )}
                          {tripHirePaymentData.trip_settlement_info.freight_charges != 0 && (
                            <CCol xs={12} md={3}>
                              <CFormLabel htmlFor="freight_deduction_charges">
                                Freight Deduction <REQ />{' '}
                              </CFormLabel>
                              <br />
                              <CFormInput
                                id="freight_deduction_charges"
                                name="freight_deduction_charges"
                                maxLength={5}
                                size="sm"
                                value={freightDeduction}
                                onChange={(e) => {deductionUpdate(e,'4')}}
                              />
                            </CCol>
                          )}
                          {tripHirePaymentData.trip_settlement_info.diversion_return_charges != 0 && (
                            <CCol xs={12} md={3}>
                              <CFormLabel htmlFor="stockDiversion_deduction_charges">
                                Stock Diversion / Return Deduction <REQ />{' '}
                              </CFormLabel>
                              <br />
                              <CFormInput
                                id="stockDiversion_deduction_charges"
                                name="stockDiversion_deduction_charges"
                                maxLength={5}
                                size="sm"
                                value={diversionReturnDeduction}
                                onChange={(e) => {deductionUpdate(e,'5')}}
                              />
                            </CCol>
                          )}
                          {tripHirePaymentData.trip_settlement_info.halting_charges != 0 && (
                            <CCol xs={12} md={3}>
                              <CFormLabel htmlFor="halting_deduction_charges">
                                Halting Deduction <REQ />{' '}
                              </CFormLabel>
                              <br />
                              <CFormInput
                                id="halting_deduction_charges"
                                name="halting_deduction_charges"
                                maxLength={5}
                                size="sm"
                                value={haltingDeduction}
                                onChange={(e) => {deductionUpdate(e,'6')}}
                              />
                            </CCol>
                          )}
                          {tripHirePaymentData.trip_settlement_info.toll_amount != 0 && (
                            <CCol xs={12} md={3}>
                              <CFormLabel htmlFor="toll_deduction_charges">
                                Toll Deduction <REQ />{' '}
                              </CFormLabel>
                              <br />
                              <CFormInput
                                id="toll_deduction_charges"
                                name="toll_deduction_charges"
                                maxLength={5}
                                size="sm"
                                value={tollDeduction}
                                onChange={(e) => {deductionUpdate(e,'7')}}
                              />
                            </CCol>
                          )}
                          <CCol xs={12} md={3}>
                            <CFormLabel htmlFor="total_deduction_amount">
                              Total Deduction
                            </CFormLabel>
                            <br />
                            <CFormInput
                              id="total_deduction_amount"
                    ...
                    <CModal
                      visible={deductionSubmit}
                      backdrop="static"
                      // scrollable
                      onClose={() => {
                        setDeductionSubmit(false)
                      }}
                    >
                      <CModalBody>
                        <p className="lead">Are you sure to Post the Deduction Details to SAP ?</p>
                      </CModalBody>
                      <CModalFooter>
                        <CButton
                          className="m-2"
                          ...
                          onClick={() => {
                            setDeductionSubmit(false)
                            TripsheetDeductionPaymentSubmit()
                          }}
                        >
                          Yes
                        </CButton>
...
export default TripsheetHireDeduction
