...
const NlmtVehicleMaintainence = () => {
  ...
  const loadVehicleMaintenanceTable = () => {
    NlmtVehicleMaintenanceService.getVehicleReadyToMaintenance().then((res) => {
  ...
  (data) =>
    ...
    Number(data.vehicle_info?.vehicle_type_id) === 21
      ...
      filterData.map((data, index) => {
        const currentPosition = Number(data.vehicle_current_position)
        const parkingStatus = Number(data.parking_status)
        rowDataList.push({
          sno: index + 1,
          Tripsheet_No: data.tripsheet_id ?? 'N/A',
          Vehicle_Type: data.vehicle_info?.vehicle_type_id ?? 'N/A',
          Vehicle_No: data.vehicle_info?.vehicle_number ?? 'N/A',
          Driver_Name: data.driver_info?.driver_name ?? 'N/A',

          Waiting_At: (
            <span className="badge rounded-pill bg-info">
              {currentPosition === Vehicle_Current_Position.GATE_IN
                ? 'Trip In'
                : currentPosition === Vehicle_Current_Position.VEHICLE_MAINTENANCE_STARTED &&
                  parkingStatus === ACTION.VEHICLE_MAINTENANCE_GATE_IN
                  ? 'Outside VM Started'
                  : currentPosition === Vehicle_Current_Position.VEHICLE_MAINTENANCE_STARTED &&
                    parkingStatus !== ACTION.VEHICLE_MAINTENANCE_GATE_IN
                    ? 'Inside VM Started'
                    : currentPosition === Vehicle_Current_Position.VEHICLE_MAINTENANCE_ENDED
                      ? 'VM Completed'
                      : 'Gate Out'}
            </span>
          ),
              ...
              data.vehicle_current_position ==
                ...
                <CButton className="badge" color="warning">
                  <Link
                    ...
                    to={`nlmtVehicleMaintainence/${data.nlmt_trip_in_id}`}
                  >
                    Maintenance Start
                  </Link>
                </CButton>
                ...
                <CButton className="badge" color="warning">
                  <Link
                    className="text-white"
                    to={`nlmtVehicleMaintainenceEnd/${data.maintenance_id}`}
                  >
                    Maintenance End
                  </Link>
                </CButton>
