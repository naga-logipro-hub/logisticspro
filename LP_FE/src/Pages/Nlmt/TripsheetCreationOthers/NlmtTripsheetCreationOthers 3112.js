...
const NlmtTripsheetCreationOthers = () => {
  ...
  const advanceEligible =
    values.advanceRequest === '1' && freightRate
      ? ((Number(freightRate) * Number(advancePercentage)) / 100).toFixed(2)
      : ''
  ...
  const handleCreateClick = async () => {
    try {
      ...
      const tripRes = await NlmtTripInService.handleTripInAction({
        ...
        driver_id: values.driverId || null,      // ✅ nullable bcoz Stored driver Name
        driver_name: values.driverName,
        driver_phone_1: values.driverMobile1,    // ✅ nullable values.driverMobile1
        vehicle_inspection_id: null,             // ✅ nullable bcoz not VI
        created_by: user_id,
      ...
      if (tsRes?.message === 'Tripsheet Creation Completed') {
        Swal.fire({
          ...
          navigate('/NlmtTripInOwnVehicle') // ✅ redirect works
        })
      }
    } catch (error) {
                ...
                {(isHire || isParty) && (
                  <CCol md={3}>
                    ...
                    <CFormInput
                      ...
                      onBlur={() => {
                        if (typedVehicleNo) {
                          NlmtVehicleMasterService.getNlmtHireVehicleCheck(typedVehicleNo).then((res) => {
                            if (res?.data?.exists) {
                              ...
                              if (Number(v.vehicle_is_assigned) === 1) {
                                ...
                                Swal.fire({
                                  ...
                                  text: 'Please check the vehicle number',
                                  icon: 'warning',
                                  confirmButtonText: 'OK',
                                }).then(() => {
