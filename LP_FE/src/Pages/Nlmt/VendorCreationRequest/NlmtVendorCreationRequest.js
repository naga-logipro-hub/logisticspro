...
const NlmtVendorCreationRequest = () => {
...
const valueAppendToImage = (image) => {
  ...
  const ShedData = (shed_id) => {
    NlmtShedMasterService.getShedById(shed_id).then((resp) => {
...
const checkAttachments = (str) => {
  if (!str) return true; // treat undefined/null/empty as "attachment missing"
  ...
  useEffect(() => {
    NlmtDocumentVerificationService.getSingleVehicleInfoOnTripIn(id).then((res) => {
      const resData = res.data.data

  if (resData?.vendor_info) {
  console.log(resData);
} else {
  console.log("Vendor info not found for this trip");
}
      ...
      values.aadharCopy = checkAttachments(resData.vendor_info?.aadhar_copy) ? '' : resData.vendor_info?.aadhar_copy
      values.panCopy = checkAttachments(resData.vendor_info?.pan_copy) ? '' : resData.vendor_info?.pan_copy
      values.bankPass = checkAttachments(resData.vendor_info?.bank_pass_copy) ? '' : resData.vendor_info?.bank_pass_copy
      values.TDSfront = checkAttachments(resData.tds_dec_form_front)
        ? ''
        : resData.tds_dec_form_front
      values.TDSback = checkAttachments(resData.vendor_info?.tds_dec_form_back) ? '' : resData.vendor_info?.tds_dec_form_back

      if (!resData.vehicle_inspection_id) {
        console.log('RMSTO YES')
        setRmsto(true)

        if (checkAttachments(resData.vendor_info?.pan_copy)) {
          setPanDel(true)
        }

        if (checkAttachments(resData.vendor_info?.aadhar_copy)) {
          setAdharDel(true)
        }

        if (checkAttachments(resData.vendor_info?.bank_pass_copy)) {
          setPassBookDel(true)
        }

        if (checkAttachments(resData.vendor_info?.tds_dec_form_front)) {
          setTdsFrontDel(true)
        }

        if (checkAttachments(resData.vendor_info?.tds_dec_form_back)) {
          setTdsBackDel(true)
        }
      ...
      ShedData(resData.vendor_info?.shed_id)
      GetBankData()
...
console.log(currentInfo,'CurrentInfo')
  ...
  const vendorInfoUpdate = (sap_pan_data) => {
    ...
    formData.append('parking_id', currentInfo.id)
    formData.append('vendor_id', currentInfo.vendor_info.id)
    ...
    NlmtVendorCreationService.updateVendorInfo(formData).then((res) => {
      setFetch(true)
      console.log(res)
      if (res.status == 200) {
        Swal.fire({
          ...
          navigation('/NlmtVendorRequestTable')
        })
  ...
  const addVendorRequest = (status) => {
    ...
    formData.append('owner_name', values.vendorName||values.vendor_info.owner_name)
    formData.append('owner_name2', values.vendorName2||values.vendor_info.owner_name2)
    ...
    NlmtVendorCreationService.updateVendorRequestData(id, formData)
      .then((res) => {
        setFetch(true)
        console.log(res)
        if (res.status == 200) {
                    ...
                    <>
                      <CCol xs={12} md={3}>
                        ...
                        <CFormInput
                          size="sm"
                          id="Vnum"
                          value={currentInfo.nlmt_vehicle_info ? currentInfo.nlmt_vehicle_info.vehicle_number : ''}
                          readOnly
                        />
                      </CCol>
                    </>
...
export default NlmtVendorCreationRequest
