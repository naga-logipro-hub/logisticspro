...
import NlmtMobileOTP from 'src/Service/Nlmt/Advance/NlmtMobileOTP'

const NlmtAdvanceCreationOwn = ({
  values,
  errors,
  handleChange,
  temp,
...
const getDeadTime = () => {
  ...
  useEffect(() => {
    NlmtDriverMasterService.getNlmtDriversById(values.driver_id).then((res) => {
      if (res.status === 200) {
...
function OTPgeneration(){
  ...
  NlmtMobileOTP.getOTP(formData).then((res) => {
  if (res.status === 200) {
    setOtpGenerated(true)
    setResendOtp(false)
    setAcceptBtn2(true)
...
const OTPMessage = (e) => {
  // e.preventDefault()
 NlmtMobileOTP.gentrateOTP(values.driver_id,values.trip_sheet_no,values.advance_paymented||values.advance_payments).then((res) => {
    if (res.status == 200) {
      setVendor(true)
      setAcceptBtn2(true)
...
function OTPVerify(otpValue){
  if (otpValue.target.value.length === 4) {
    ...
    console.log('res')
    NlmtMobileOTP.VerifyOTP(formData).then((res) => {
      console.log('res')
      ...
      <CRow className="">
        ...
        <CCol xs={12} md={3}>
          ...
          <CFormInput
            size="sm"
            id="vCap"
            value={vehicleCapacityName+' Mts'+' & '+vehicleBodyName}
            readOnly
          />
        </CCol>
        <CCol xs={12} md={3}>
        ...
        <CCol xs={12} md={3}>
          ...
          />
            ...
            <CCol xs={12} md={3}>
              ...
              <CFormInput
                ...
                type="text"
                value={singleVehicleInfo.tripsheet_info.inspection_info.inspection_time_string}
                readOnly
              />
            </CCol>
          <CCol xs={12} md={3}>
          ...
          <CCol xs={12} md={3}>
            ...
            <CFormInput
              ...
              onBlur={onBlur}
              onChange={handleChange}
              value={singleVehicleInfo.tripsheet_info.advance_amount}
              id="advance"
              type="text"
              maxLength={6}
              readOnly
          ...
          {(values.advance_payments == 0 && values.advance_paymented == 0) ||(values.advance_payments == 0 && values.advance_paymented == undefined) ||
          (values.advance_payments > 0 && values.advance_paymented == 0) ||
        ...
        <CCol xs={12} md={3}>
          ...
          {/* {values.otp && */}
          {otpGenerated && otpVerify == true && (values.advance_payments > 0 && values.advance_paymented > 0 || values.advance_payments == 0 && values.advance_paymented > 0 || values.advance_payments > 0 && values.advance_paymented == undefined || values.advance_payments == 0 && values.advance_paymented == undefined) &&
          <CCol xs={12} md={3}>
        ...
        <CCol xs={12} md={3}>
          ...
          <CFormInput
            ...
            //  value={singleVehicleInfo.vehicle_inspection_trip.inspection_time_string}
             readOnly
            value={formatDate(singleVehicleInfo.tripsheet_info.expected_delivery_date || '')}
          />
        </CCol>
        <CCol xs={12} md={3}>
          ...
          <CFormInput
            ...
            onBlur={onBlur}
            onChange={handleChange}
            value={formatDate(singleVehicleInfo.tripsheet_info.expected_return_date_time || '')}
            readOnly
          />
        </CCol>
        {/* <CCol xs={12} md={3}>
...
export default NlmtAdvanceCreationOwn
