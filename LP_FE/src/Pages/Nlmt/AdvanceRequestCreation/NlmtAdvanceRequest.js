...
import AdvanceCreationOwn from './segments/OwnAndContract/NlmtAdvanceCreationOwn'
import AdvanceCreationHire from './segments/Hire/NlmtAdvanceCreationHire'
...
const NlmtAdvanceRequest = () => {
  ...
  function CreateAdvance(status) {
    ...
    let sap_advance_payments = values.advance_payments

    formData.append('LIFNR', values.vendor_code ? singleVehicleInfo.vendor_info.vendor_code : values.vendor_code)
    formData.append('TRIP_SHEET', values.trip_sheet_no)
    // formData.append('TRIP_SHEET', 'OK76913')
    formData.append('VEHICLE_NO', singleVehicleInfo.vehicle_info.vehicle_number ? singleVehicleInfo.vehicle_info.vehicle_number : singleVehicleInfo.vehicle_number)
    ...
    if (
      bankAmountExists(
        Number(updated_freight),
        Number(values.advance_payments),
        Number(singleVehicleInfo.trip_sheet_info.advance_payment_diesel)
      ) === 101
    ) {
      setFetch(true)
    ...
    if (values.actual_freight > "0" && values.advance_payment_diesel == "0") {
      ...
      NlmtAdvanceHireSAP.AdvanceHireSAP(formData).then((res) => {
        ...
        data.append('actual_freight', singleVehicleInfo.tripsheet_info.trip_vehicle_route.freight_rate)
        data.append('vendor_code', values.vendor_code)
        data.append('vendor_outstanding', values.driver_outstanding)
        data.append('advance_payments', values.advance_payments)
        ...
        NlmtAdvanceCreationService.createAdvance(data).then((res) => {
          if (res.status === 200) {
            ...
            if (values.advance_payments > "0" && values.advance_payments > "0") {
              Swal.fire({
      ...
      NlmtAdvanceHireSAP.AdvanceHireSAP(formData).then((res) => {
        ...
        NlmtAdvanceCreationService.createAdvance(data).then((res) => {
          if (res.status === 200) {
            console.log('NlmtAdvanceCreationService values')
            console.log(res)
            setFetch(true)
            setAcceptBtn(true)
            setButtonDisable(true)
  ...
  function CreateAdvanceHire(status) {
    ...
    formData.append('TAX_TYPE', values.gst_tax_type && values.gst_tax_type !== 'Empty' ? values.gst_tax_type : safeEmpty)
    formData.append('TDS', values.vendor_tds && values.vendor_tds !== '0' ? 'YES' : safeEmpty)
    formData.append('TDS_VALUE', values.vendor_tds && values.vendor_tds !== '0' ? values.vendor_tds : safeEmpty)
    formData.append('REMARKS', remarks)
    formData.append('LIFNR', singleVehicleInfo.vendor_info.vendor_code)
    // formData.append('FREIGHT_PAYMENT', Number(values.advance_payments) || 1000)
    formData.append('FREIGHT_PAYMENT', `FREIGHT_PAYMENT:${Number(totalFreightAmount) || 0}`)

    formData.append('POST_DATE', values.sap_invoice_posting_date)
    if (Number(values.advance_payments) > 0) {
      formData.append('BANK_DATE', values.bank_date)
    }
    formData.append('REF_NO', values.supplier_ref_no ? values.supplier_ref_no : values.trip_sheet_no)
    formData.append('REF_DATE', values.sap_invoice_posting_date)
    formData.append('BANK_PAYMENT', Number(values.advance_payments))
  ...
  useEffect(() => {
    if (!errors.advance_form && !errors.advance_form) {
      setAcceptBtn(false)
    } else {
      setAcceptBtn(true)
    }
