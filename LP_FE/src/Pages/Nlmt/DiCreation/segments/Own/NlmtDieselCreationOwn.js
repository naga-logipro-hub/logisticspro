...
useEffect(() => {
  if (singleVehicleInfo?.driver_info) {
    values.driver_id = singleVehicleInfo.driver_info.driver_id
    values.driveMobile = singleVehicleInfo.driver_info.driver_phone_1
  }
}, [singleVehicleInfo])
  useEffect(() => {
  ...
  const selectedDriver = driverMasterData.find(
    (d) => String(d.driver_id) === String(selectedDriverId)
  )

  if (selectedDriver) {
    values.driveMobile = selectedDriver.driver_phone_1
...
console.log(driverMasterData, 'driverMasterData')
useEffect(() => {
  if (!mastersLoaded) return
}, [mastersLoaded, singleVehicleInfo])
  return (
    <>
      <CRow className="">
        <CCol xs={12} md={3}>
...
value={
    mastersLoaded
      ? getDefinitionName(vehicleType, singleVehicleInfo?.vehicle_info?.vehicle_type_id)
      : ''
  }
            readOnly
          />
  ...
  value={
    mastersLoaded
      ? getDefinitionName(vehicleCapacity, singleVehicleInfo?.vehicle_info?.vehicle_capacity_id)
      : ''
  }
            readOnly
          />
        ...
        {singleVehicleInfo?.vehicle_inspection_status && (
          <CCol xs={12} md={3}>
    ...
    <option key={driver.driver_id} value={driver.driver_id}>
      {driver.driver_name}
        ...
        <CCol xs={12} md={3}>
          ...
          <CFormInput size="sm" id="driveMobile"    value={values.driveMobile || driverPhoneNumberById || ''} readOnly />
        </CCol>
        <CCol xs={12} md={3}>
...
export default NlmtDieselCreationOwn
