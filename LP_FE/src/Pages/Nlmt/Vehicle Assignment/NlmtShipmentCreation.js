...
import NlmtDeliveryOrderInfo from './Segments/NlmtDeliveryOrderInfo'
...
const NlmtShipmentCreation = () => {
  ...
  function checkModalDisplay() {
    ...
    console.log(shipment_details2, 'shipment_details2')
  ...
  const deleteShipment = () => {
    ...
    console.log(deletedShipmentInfo, "deletedShipmentInfo")
    var vehicleType = ''

    /* Depo Party Locations 2-Trichy, 3-Chennai, 4-Kanyakumari, 5-Madurai */
    var depo_party_location_array = [2, 3, 4, 5, 13, 14, 15, 16, 18, 20]
    ...
    if (deletedShipmentInfo.vehicle_type_id == '1') {
      ...
      if (deletedShipmentInfo.vehicle_type_id == '4' && deletedShipmentInfo.vehicle_others_type == '2') {
        vehicleType = 'D2R'
      } else if (inArray(values.vehicle_location_id, depo_party_location_array)) {
        vehicleType = 'DEPO_PARTY'
      } else if (inArray(values.vehicle_location_id, foods_party_location_array)) {
    ...
    if (deletedShipmentInfo.vehicle_type_id != '4' && ship_parent_not_having && (deletedShipmentInfo.trip_sheet_info.sap_flag == '1' || deletedShipmentInfo.trip_sheet_info.sap_flag == '2')) {
      ...
      SAPData.append('TRIP_SHEET', deletedShipmentInfo.trip_sheet_info.nlmt_tripsheet_no)
      ...
      TripSheetInfoService.StartTSInfoToSAP(SAPData).then((response) => {
        ...
        // formDataForSAPShipmentDelete.append('sap_flag','0')
        NlmtVehicleAssignmentSapService.deleteShipment(formDataForSAPShipmentDelete).then((res) => {
          console.log(res)
          ...
          if (sap_shipment_status == '1' && res.status == 200 && sap_shipment_no && sap_ts_shipment) {
            ...
            NlmtVehicleAssignmentService.deleteShipmentOrder(shipmentToDelete).then((res) => {
              setFetch(true)
              if (res.status == 204) {
      ...
      formDataForSAPShipmentDelete.append('shipment_no', shipmentToDelete)
      formDataForSAPShipmentDelete.append('trip_sheet', shipmentTSToDelete)
      NlmtVehicleAssignmentSapService.deleteShipment(formDataForSAPShipmentDelete)
        .then((res) => {
          let sap_shipment_no = res.data.SHIPMENT_NO
          let sap_shipment_status = res.data.STATUS
          let sap_ts_shipment = res.data.TRIP_SHEET
          ...
          if (sap_shipment_status == '1' && res.status == 200 && sap_shipment_no && sap_ts_shipment) {
            NlmtVehicleAssignmentService.deleteShipmentOrder(shipmentToDelete)
              .then((res) => {
                setFetch(true)
                if (res.status == 204) {
                  toast.success('Shipment Deleted Successfully!')
  ...
  const pgiUpdateInDb = (json_data) => {
    ...
    NlmtVehicleAssignmentService.updatePgiInfoToDb(json_data).then((res) => {
      console.log(res)
    })
  }
  ...
  const deliveryQuantityUpdateInDb = (json_data) => {
    // console.log(json_data)
    NlmtVehicleAssignmentService.updateDeliveryQuantityInfoToDb(json_data).then((res) => {
      console.log(res)
    })
  }
  ...
  const secondWeightUpdateInDb = (json_data) => {
    // console.log(json_data)
    NlmtVehicleAssignmentService.updateSecondWeightInfoToDb(json_data).then((res) => {
      console.log(res)
    })
  }
  ...
  const loadSaleOrderInfoTable = () => {
    ...
    obj.forEach(([key_st, value]) => {
      ...
      if (!inArray(value.WERKS, nlcdPlantsArrayData) && inArray(value.WERKS, user_access_locations) && inArray(value.INCO1, user_inco_terms)) {
        // if (value.WERKS == '1009') {
        del_order.push({
          sno: (key_st % 10) + 1,
  ...
  useEffect(() => {
    ...
    NlmtVehicleAssignmentSapService.getSaleOrders()
      .then((res) => {
    ...
    // NlmtVehicleAssignmentSapService.getPGIInfoData()
    //   .then((res) => {
    //     console.log(res.data)
    ...
    NlmtVehicleAssignmentSapService.getDeliveryQuantityInfoData()
      .then((res) => {
        console.log(res.data)
        let response_info = res.data
        if (response_info.length > 0) {
    ...
    NlmtVehicleAssignmentSapService.getSecondWeightInfoData()
      .then((res) => {
        console.log(res.data)
        let response_info = res.data
        if (response_info.length > 0) {
  ...
  const getShipmentData = (shipmentNo) => {
    ...
    setSmallFetch(false)
    NlmtVehicleAssignmentSapService.getShipmentInfoForUpdation(data).then((res) => {
  ...
  const TripsheetShipmentUpdate = () => {
    ...
    console.log(tripHirePaymentData.trip_sheet_info.nlmt_tripsheet_no)
    ...
    formData.append('transport_plant', 'FD00')
    ...
    formData.append('tripsheet_id', tripHirePaymentData.tripsheet_id)
    formData.append('assigned_by', 1)
    formData.append('tripsheet_no', tripHirePaymentData.trip_sheet_info.nlmt_tripsheet_no)
    ...
    NlmtVehicleAssignmentService.insertShipmentOrder(formData).then((res) => {
  ...
  const invoiceCompletionCheck = (Shipment_Data) => {
    ...
    formDataForSAPInvoiceComplete.append('tripsheet_no', Shipment_Data.trip_sheet_info.nlmt_tripsheet_no)
    ...
    NlmtVehicleAssignmentSapService.updateShipmentInvoiceCompletionCheck(formDataForSAPInvoiceComplete)
      .then((res) => {
        ...
        if (res.status == 200) {
          ...
          if (sap_response.STATUS == 1) {
            ...
            NlmtVehicleAssignmentService.updateSecondWeightInfoToDbData(sap_response_data).then((res) => {
              console.log(res)
              setFetch(true)
              if (res.status == 200) {
                Swal.fire({
  ...
  useEffect(() => {
    if (shipmentToDelete != '') {
      NlmtVehicleAssignmentService.getSingleShipment(shipmentToDelete).then((res) => {
        console.log(res.data.data, 'getSingleShipment-todelete')
        setDeletedShipmentInfo(res.data.data)

        NlmtVehicleAssignmentService.getShipmentInfoByPId(res.data.data.parking_id).then((res) => {
  ...
  const loadShipmentViewTable = () => {
    NlmtVehicleAssignmentService.getAllShipmentOrders().then((res) => {
      // console.log(res)
      // setFetch(true)
      ...
      console.log(filterData, 'filterData-getAllShipmentOrders')
      let temp = 0
      let temp_invoice = 0
      filterData.map((data, index) => {
        if (data.assigned_by == 1) {
          rowDataList.push({
            sno: temp + 1,
            Vehicle_No: data.vehicle_number,
            Tripsheet_No: data.trip_sheet_info?.nlmt_tripsheet_no,
            ...
            Action: (
              <span className="float-start" color="danger">
                <CButton className="btn btn-secondary btn-sm me-md-1">
                  <Link className="text-white" to={`NlmtShipmentInfo/${data.shipment_no}`}>
                ...
                <CButton
                  ...
                  onClick={() => {
                    ...
                    setShipmentTSToDelete(data.trip_sheet_info?.nlmt_tripsheet_no)
                  }}
...
console.log(data,'datadatadatadatadatadatadata')
        if (data.assigned_by == 1 && data.shipment_pgi_status == 1) {
    ...
    NlmtVehicleAssignmentService.getMissingShipmentOrders().then((res) => {
      // setFetch(true)
      let missingData = res.data
      console.log(missingData, 'getMissingShipmentOrders')
      let rowSDataList = []
      const filterSData = missingData.filter(
        (data) => user_locations.indexOf(data.pyg_resource.vehicle_location_id) != -1
      )
  ...
  function createShipment() {
    console.log(values,'createShipment Values')
    console.log(shipmentData, 'createShipment-shipmentData')
    ...
    if (values.vehicle_type_id == '21') {
      vehicleType = 'OWN'
    } else if (values.vehicle_type_id == '22') {
      vehicleType = 'HIRE'
    } else if (values.vehicle_type_id == '23') {
      vehicleType = 'PARTY'
    }

    /* Set Shipment Data via Selected Deliveries by Loop */
    if (!values.tripsheet_id || !values.tripsheet_no) {
  toast.error("Tripsheet not assigned. Please reselect vehicle.")
  return
}
    for (var i = 0; i < shipmentData.length; i++) {
      ShipmentCreationSendData.SIGNI = values.vehicle_number
      ShipmentCreationSendData.trip_sheet = values.tripsheet_no ?? null
    ...
    NlmtVehicleAssignmentSapService.createShipment(ShipmentCreationSendData_seq).then((res) => {
      console.log(res)
      let sap_shipment_no = res.data.SHIPMENT_NO
      let sap_shipment_status = res.data.STATUS
      let sap_ts_shipment = res.data.TRIP_SHEET
      ...
      if (sap_shipment_no && sap_ts_shipment) {
        ...
        NlmtVehicleAssignmentService.createShipmentOrder(formData)
          .then((res) => {
            setFetch(true)
            if (res.status === 200) {
    ...
    .catch((error) => {
      ...
      if (error.response) {
        ...
        toast.error('Server Error: ' + (error.response.data.message || 'Failed to create shipment'))
...
export default NlmtShipmentCreation
