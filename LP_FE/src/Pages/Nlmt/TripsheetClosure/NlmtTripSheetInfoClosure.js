...
import NlmtRJSOJourneyInfo from './JourneyInfo/NlmtRJSOJourneyInfo'
...
import RakeJourneyInfo from './JourneyInfo/NlmtRakeJourneyInfo'
...
import OthersJourneyInfo from './JourneyInfo/NlmtOthersJourneyInfo'
...
const NlmtTripSheetInfoClosure = () => {
  ...
  useEffect(() => {
    ...
    setTotalTripPL(
      parseFloat(
        trip_income - Number(tripsettlementData?.expense || 0)
      ).toFixed(2)
    )
  ...
  useEffect(() => {
    if (id) {
      NlmtTripSheetClosureService.getTripSettlementInfoByParkingId(id).then((res) => {
        console.log(res.data.data[0], 'NlmtTripSheetClosureService.getTripSettlementInfoByParkingId')
        if (res.data.data && res.data.data[0]) {
          const settlement = res.data.data[0]
          setTripsettlementData(settlement)
          values.remarks = settlement.remarks || ''
          values.income_remarks = settlement.income_remarks || ''
        } else {
  ...
  fetchedData?.parking_info?.tripsheet_info?.trip_vehicle_info?.vehicle_type_id
)
  ...
  const actualFreight = Number(
    fetchedData.advance_payment_info.actual_freight || 0
  )

  const lowTonnage = Number(
    fetchedData.advance_payment_info.low_tonnage_charges || 0
  )

  values.freight_charges = actualFreight - lowTonnage
  values.low_tonnage_charges = lowTonnage
}
...
else if (vehicleTypeId === 23) {
  setMainKey(2)   // Party same as Own
}
  ...
  const totalChargesFinder = () => {
    let total_charge = 0
    if (Number(tripInfo?.trip_vehicle_info?.vehicle_type_id) === 22) {
      total_charge =
  ...
  const totalChargesCalculator = (data) => {
    ...
    if (Number(tripInfo?.trip_vehicle_info?.vehicle_type_id) === 22) {
      ...
      total_charge =
        ...
        Number(data.diversion_return_charges ? data.diversion_return_charges : 0) +
        Number(rvTotalValuesBP?.rvTotalDieselAmount || 0) +
Number(urvTotalAmountFinder || 0) +
Number(ExpenseUnloadingCharges || 0)
    }

    if (tripInfo?.rj_so_info?.length > 0) {
      let rjso_expenses =
        Number(data.rjso_tarpaulin_charges || 0) +
        Number(data.rjso_weighment_charges || 0) +
        Number(data.rjso_unloading_charges || 0) +
        Number(data.rjso_loading_charges || 0) +
        Number(data.rjso_bata_amount || 0) +
        Number(data.rjso_commision_charges || 0) +
        Number(data.rjso_misc_charges || 0) +
        Number(data.rjso_munic_charges || 0) +
        Number(data.rjso_halt_charges || 0) +
        Number(data.rjso_en_diesel_charges || 0)
  ...
  const sendDriverExpenseToSAP = () => {
    ...
    TripSheetClosureSapService.driverExpensePost(sapData).then((res) => {
      ...
      if (sap_ts_no != '' && sap_status == 1 && sap_expense_post_document != '') {
        ...
        Swal.fire({
          title: Number(tripInfo?.vehicle_info?.vehicle_type_id) === 22 ? `Vendor Expense Posted Document Cannot Be Created From SAP due to ${sap_expense_post_message}.. Kindly Contact SAP Admin!` : 'Driver Expense Posted Document Cannot Be Created From SAP.. Kindly Contact SAP Admin!',
          icon: "warning",
  ...
  const addIncomeClosureRequest = (status, sap_expense_post_document = '') => {
    if (status == '2') {
      ...
      NlmtTripSheetClosureService.updateIncomeClosureRejection(id, formData)
        .then((res) => {
          // alert('success')
          setFetch(true)
          if (res.status == 200) {
      ...
      NlmtTripSheetClosureService.updateIncomeClosureAcception(id, formData)
        .then((res) => {
          // alert('success')
          setFetch(true)
                  ...
                  <CTabPane role="tabpanel" aria-labelledby="profile-tab" visible={activeKey === 15}>
                    ...
                    {tripStoData && tripStoData.length > 0 && (
                      <>
                        ...
                        <NlmtRMSTOJourneyInfo rmstoJourneyInfo={tripStoData} title="STO Information" />
                        <hr />
                      </>
                    )}
                  ...
                  >
                    ...
                    {tripStoData && tripStoData.length > 0 && JourneyInfoExists(3, tripStoData) && (
                      <>
                        ...
                        <NlmtFCIJourneyInfo rmstoJourneyInfo={tripStoData} title="FCI Information" />
                        <hr />
                      </>
                    )}
                    {tripOthersStoData && tripOthersStoData.length > 0 && (
