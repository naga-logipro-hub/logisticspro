...
const NlmtRakeJourneyInfo = (props) => {
  ...
  const getClosureSTOData = () => {
    ...
    stoTripData.map((data, index) => {
      if (data.trip_driver_id) {
        if(data.sto_delivery_no == '' && data.rm_type == 2){
          rowDataList.push({
            sno: index + 1,
            ...
            Rake_Div: data.rm_type == '1' ? 'FOODS' : 'MMD',
...
NlmtRakeJourneyInfo.propTypes = {
  rmstoJourneyInfo: PropTypes.array.isRequired,
  title: PropTypes.string,
}

NlmtRakeJourneyInfo.defaultProps = {
  title: '',
}
export default NlmtRakeJourneyInfo
