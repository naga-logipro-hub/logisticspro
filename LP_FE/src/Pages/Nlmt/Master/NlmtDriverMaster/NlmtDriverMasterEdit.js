...
const driverAlreadyExistInDriverMaster = (code) => {
  return driverMasterData.some(
    (d) =>
      d.driver_code === code && d.id !== singleDriver.id
  )
}
const isUpdateDisabled = values.licenseValidityStatus !== 'Yes'
  function addNewDriver() {
    const formData = new FormData()

  if (
  values.driverCode !== singleDriver.driver_code &&
  driverAlreadyExistInDriverMaster(values.driverCode)
) {
  toast.warning('Updated Driver Code already exists!')
  return
}
    setFetch(false)
    ...
    // console.log(values)
    NlmtDriverMasterService.updateNlmtDrivers(id, formData)
      .then((res) => {
        setFetch(true)
        if (res.status === 200) {
          ...
          setTimeout(() => {
            navigation('/NlmtDriverMasterTable')
          }, 1000)
        }
      })
      .catch((error) => {
  ...
  useEffect(() => {
    ...
    NlmtDriverMasterService.getNlmtDriversById(id).then((res) => {
      setFetch(true)
    ...
    values.licenseValidityStatus =
  res.data.data.license_validity_status == 1 ? 'Yes' : 'No'
...
export default NlmtDriverMasterEdit
