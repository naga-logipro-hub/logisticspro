...
const StoTableRMSTOComponent = (props) => {
  console.log(props.stoTableDataRMSTO)
  const onEdit = (index) => {
    props.onEdit(index)
  }
  ...
  return (
    <>
      <div>
        <h3>{props.title} </h3>
        {props.stoTableDataRMSTO.length ? (
          <CTable className="table table-sm" responsive="xxl">
            ...
            <CTableBody>
              {props.stoTableDataRMSTO.map((stoData, index) => (
                <tr key={`stoTableDataRMSTO${index}`}>
                  <td>{stoData.sto_delivery_number_rmsto || stoData.sto_delivery_no}</td>
                  <td>{stoData.sto_po_number_rmsto || stoData.sto_po_no}</td>
                  <td>{stoData.sto_delivery_division_rmsto || stoData.sto_delivery_division}</td>
                  <td>
                    {location_name(stoData.sto_from_location_rmsto) ||
                      location_name(stoData.from_location)}
                  </td>
                  <td>
                    {location_name(stoData.sto_to_location_rmsto) ||
                      location_name(stoData.to_location)}
                  </td>

                  <td>{stoData.sto_delivery_quantity_rmsto || stoData.sto_delivery_quantity}</td>
                  <td>{stoData.sto_freight_amount_rmsto || stoData.freight_amount}</td>
                  <td>{stoData.sto_delivery_date_time_rmsto || stoData.delivered_date_time}</td>
                  ...
                  <td>
                    <button
                      type="button"
                      disabled={props.isStoEditModeRMSTO}
...
StoTableRMSTOComponent.propTypes = {
  stoTableDataRMSTO: PropTypes.array.isRequired,
  title: PropTypes.string,
  onEdit: PropTypes.func.isRequired,
  onDelete: PropTypes.func.isRequired,
}

StoTableRMSTOComponent.defaultProps = {
  title: '',
}
export default StoTableRMSTOComponent
