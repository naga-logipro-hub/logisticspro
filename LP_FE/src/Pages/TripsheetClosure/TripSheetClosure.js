...
const TripSheetClossure = () => {
  ...
  useEffect(() => {
    if (id) {
      TripSheetClosureService.getTripSettlementInfoByParkingId(id).then((res) => {
        console.log(res.data.data[0],'getTripSettlementInfoByParkingId')
        ...
        if (res.data.data[0] === undefined) {
          ...
          if(res.data.data[0].freight_charges){
            ...
            if(tripInfo.advance_payment_info && tripInfo.advance_payment_info.low_tonnage_charges && Number(tripInfo.advance_payment_info.low_tonnage_charges) > 0){
      ...
      RJSaleOrderCreationService.getWaitingRJRequestsbyParkingId(id).then((res) => {
  ...
  useEffect(() => {
    ...
    TripSheetClosureService.getVehicleInfoById(id)
      .then((res) => {
        ...
        console.log(fetchedData,'getVehicleInfoById')

        if(user_info.is_admin == 0 && (fetchedData.vehicle_current_position == 26 || fetchedData.vehicle_current_position == 27 || fetchedData.vehicle_current_position == 28)){
        ...
        //   : setShipmentNumber('')
        console.log(res.data.data,'getTripInfoById')
        ...
        if(fetchedData.shipment_info && fetchedData.shipment_info.length > 0){
          let ifi = fetchedData.shipment_info[0].incoterm_freight_info
          console.log(ifi,'ifi')
          setIncotermFreightInfo(ifi)
        } else {
          setIncotermFreightInfo([])
        }

        if(fetchedData.vehicle_type_id.id == '3' && fetchedData.advance_info) {
          // values.freight_charges = fetchedData.advance_info.actual_freight ? fetchedData.advance_info.actual_freight : ''
          
          if(fetchedData.advance_payment_info && fetchedData.advance_payment_info.low_tonnage_charges && Number(fetchedData.advance_payment_info.low_tonnage_charges) > 0){
            values.freight_charges = Number(fetchedData.advance_payment_info.actual_freight) - Number(fetchedData.advance_payment_info.low_tonnage_charges)
          } else if(fetchedData.advance_payment_info && (fetchedData.advance_payment_info.low_tonnage_charges || fetchedData.advance_payment_info.actual_freight)){
            values.freight_charges = fetchedData.advance_payment_info.actual_freight
          } else {
            values.freight_charges = ''
          }
          
          values.low_tonnage_charges = fetchedData.advance_info.low_tonnage_charges ? fetchedData.advance_info.low_tonnage_charges : ''
        }

        if(fetchedData.vehicle_type_id.id == '3' && !fetchedData.advance_payment_info && idt.length > 0 && fetchedData.trip_sheet_info.purpose == 1 && fetchedData.trip_sheet_info.to_divison == 1){
        ...
        console.log(dieselCollectionInfo, 'dieselCollectionInfo')
        console.log(dieselCollectionInfo.length, 'dieselCollectionInfo Length')
        ...
        fetchedData.shipment_info.length > 0 ? setfg_sales_enable(true) : setfg_sales_enable(false)
        ...
        fetchedData.trip_sheet_info.purpose == 4 ? setOthersAvailable(true) : setOthersAvailable(false)
        setFetch(true)
      })
      .catch((error) => {
        setFetch(true)
        ...
        shp.shipment_child_info.map((vv,kk)=>{ 
          if(JavascriptInArrayComponent(vv.inco_term_id,shp_incoterm_array)){
            ...
            incoTableData.filter((data)=> (data.inco_term_id == vv.inco_term_id)).map((v1,k1)=>{ 
              ...
              v1.amount = v1.amount + freightamountfinder(vv.inco_term_id,tripInfo.trip_sheet_info.freight_rate_per_tone,getDeliveryQuantity(vv.invoice_net_quantity,vv.invoice_uom))   
            ...
            valarray.amount = freightamountfinder(vv.inco_term_id,tripInfo.trip_sheet_info.freight_rate_per_tone,getDeliveryQuantity(vv.invoice_net_quantity,vv.invoice_uom))
  ...
  useEffect(() => {
    if(tripInfo && tripInfo.trip_sheet_info && tripInfo.diesel_intent_collection_info.length > 0) {
      console.log(tripInfo,'tripInfo')

      let toll_start_time = tripInfo.trip_sheet_info ? tripInfo.trip_sheet_info.tripsheet_creation_time_string : ''
      console.log(toll_start_time,'toll_start_time')


      let toll_end_time = tollEndTimeCalculator(tripInfo.diesel_intent_collection_info)
      console.log(toll_end_time,'toll_end_time')

      let too_form_data = new FormData()
      too_form_data.append('start_time', toll_start_time)
      too_form_data.append('end_time', toll_end_time)
      too_form_data.append('vehicle_id', tripInfo.vehicle_id)
      let data_string = toll_start_time+'||'+toll_end_time+'||'+tripInfo.vehicle_id
      TripSheetClosureSapService.getFasttagEntriesByVehicleId(data_string).then((res) => {
        let fasttagdata = res.data.data
        let fasttagamount = 0
        console.log(fasttagdata,'getFasttagEntriesByVehicleId')
        setFasttagData(fasttagdata)
        fasttagdata.map((child, child_index) => {
          fasttagamount += Number(child.settlement_amount)
        })
        console.log(fasttagamount,'fasttagamount')
        setFasttagAmount(fasttagamount)
      })
    }
  },[tripInfo])
  ...
  const getStoDeliveryData = (e) => {
    ...
    StoDeliveryDataService.getFgstoDeliveryData(stoValues.sto_delivery_number).then((res) => {
  ...
  useEffect(() => {
    if (tripInfo && tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.trip_sheet_no) {
      console.log(tripInfo.trip_sheet_info.purpose,'tripInfo.trip_sheet_info.purpose')
      let trip_sap_flag = tripInfo.trip_sheet_info.sap_flag
      let trip_pp_flag = tripInfo.trip_sheet_info.pp_flag
      console.log(`${trip_sap_flag}||${trip_pp_flag}`, 'trip_sap_flag||trip_pp_flag')

      // if(tripInfo.trip_sheet_info.purpose == '3'){
        StoDeliveryDataService.getRmstoDeliveryData(tripInfo.trip_sheet_info.trip_sheet_no).then(
          (res) => {
      ...
      // if(trip_sap_flag == '1'){
        StoDeliveryDataService.getAllFgstoDeliveryData(tripInfo.trip_sheet_info.trip_sheet_no).then(
          (res) => {
            let sap_response = res.data
      ...
      // }
      StoDeliveryDataService.getFciDeliveryData(tripInfo.trip_sheet_info.trip_sheet_no).then(
        (res) => {
          let sap_response = res.data
  ...
  const getRmstoDeliveryData = (e) => {
    ...
    StoDeliveryDataService.getRmstoDeliveryData(delivery_no).then((res) => {
      let sap_response = res.data[0]
      let sap_vehicle_no = sap_response.SIGNI
      let sap_po_number = sap_response.EBELN
      let sap_delivery_number = sap_response.VBELN
  ...
  /* TabFCISuccess Setup */
  useEffect(() => {
    if (stoTableDataFCI && tripInfo && tripInfo.trip_sheet_info) {
      ...
      stoTableDataFCI.length == 0 &&
        tripInfo.trip_sheet_info.purpose != '5'
          ? true
          : false
  ...
  const totalvaluefinder = (type,data) => {
    ...
    if(data){
      ...
      children.map((vv,kk)=>{
        ...
        let ammt = freightamountfinder(vv.inco_term_id,tripInfo.trip_sheet_info.freight_rate_per_tone,getDeliveryQuantity(vv.invoice_net_quantity,vv.invoice_uom))
  ...
  const trip_info_find = (info_type) => {

    console.log(info_type, 'trip_info_find-info_type')

    if (info_type == 'purpose') {
      if (tripInfo.trip_sheet_info.purpose == '1') {
        return 'FG_SALES'
      } else if (tripInfo.trip_sheet_info.purpose == '2') {
        return 'FG_STO'
      } else if (tripInfo.trip_sheet_info.purpose == '3') {
        return 'RM_STO'
      } else if (tripInfo.trip_sheet_info.purpose == '4') {
        return 'OTHERS'
      }
    }

    if (info_type == 'division') {
      if (tripInfo.trip_sheet_info.to_divison == '2') {
        return 'NLCD'
      } else {
        return 'NLFD'
      }
  ...
  const tollEndTimeCalculator = (dieselData) => {
    let end_time = ''

    let filterData =  dieselData.filter((data) =>data.diesel_type == 0)

    console.log(filterData)

    end_time = filterData[0].di_creation_time_string
 ...
 const getHSNTypeDesc = (hsn) => {
  ...
  useEffect(() => {
    ...
    if (stoOthersTableData && tripInfo && tripInfo.trip_sheet_info) {
      ...
      stoOthersTableData.length === 0 &&
        others_tripAddonAvailability == 2 &&
  ...
  useEffect(() => {
    if(tripInfo && tripInfo.trip_sheet_info){
        ...
        let filterData =  vrList.filter(
            (data) => data.vr_tr_no == null || data.vr_tr_no ==  tripInfo.trip_sheet_info.trip_sheet_no
        )
...
//   },[tripInfo, tripInfo.trip_sheet_info])
  ...
  const addOthersEnable = (data) => {
    ...
    if (
          ...
          (data.others_sto_vr_id != '' || tripInfo.vehicle_type_id.id == 3 ) &&
  ...
  const getOthersSapData = (e) => {
    ...
    StoDeliveryDataService.getOthersDeliveryData(stoOthersValues.others_sto_doc_number,values.ot_process_type).then((res) => {
      setFetch(true)
      let sap_response = res.data[0]
  ...
  const vrNotMappingSapDoc = () => {
    ...
    let filter_data =  vrData.filter(
      (data) => data.vr_tr_no ==  tripInfo.trip_sheet_info.trip_sheet_no
    )
  ...
  /* =========== Others Tripsheet Reworks Part End ask2=========== */
  ...
  const TripsheetClosureSubmit = (process) => {
    ...
    if(!(tripInfo.advance_info || tripInfo.vehicle_type_id.id != '3')){
      if(values.TdsHaving == ''){
    ...
    let actual_km_value = tripInfo.odometer_km
      ? tripKMFinder(tripInfo.odometer_km, tripInfo.odometer_closing_km)
      : ''
    formData.append('actual_km', actual_km_value)

    console.log(actual_km_value)
    formData.append('actual_mileage', calculationValues.actualMileage)
    ...
    formData.append(
      'expense',
      tripInfo.vehicle_type_id.id == 1 ? totalChargesOwn : totalChargesHire
    )
    ...
    formData.append(
      'low_tonage_charges',
      tripInfo.vehicle_type_id.id == 3 ? totalChargesOwn : totalChargesHire
    )
    ...
    formData.append('parking_id', tripInfo.parking_yard_gate_id)
    ...
    formData.append('tripsheet_id', tripInfo.tripsheet_sheet_id)
    // formData.append('tripsheet_is_settled', values.vehicleNumber)
    formData.append('tripsheet_no', tripInfo.trip_sheet_info.trip_sheet_no)
    ...
    if (tripInfo.vehicle_type_id.id == '3') {
      ...
      formData.append('sap_text', values.sap_text ? values.sap_text : '')
      if(stoOthersTableData && stoOthersTableData.length > 0){
        stoOthersTableData[0]['others_sto_vr_id'] = tripInfo.trip_sheet_info.vehicle_requests
      }
    ...
    formData.append('vehicle_no', tripInfo.vehicle_number)
    ...
    if(tripInfo && tripInfo.vehicle_type_id.id === 1 && tripInfo.rj_so_info && tripInfo.rj_so_info.length > 0) {
    ...
    if(tripInfo.vehicle_type_id.id == '1'){
    ...
    if (process == 'submit') {
      ...
      if((tripInfo.vehicle_type_id.id == 1 || tripInfo.vehicle_type_id.id == 2) && stoTableDataFCI && stoTableDataFCI.length > 0){
        /* =============================================================== */
        ...
        for (var i = 0; i < stoTableDataFCI.length; i++) {
          ...
          SentVAInfoData.TRIPSHEET_NO = tripInfo.trip_sheet_info.trip_sheet_no
        ...
        FCITripsheetSapService.sentVAInfoToSAP(SentVAInfoData_seq).then((res) => {
          ...
          let sap_res = res.data ? res.data[0] : '' 
          console.log(sap_res,'sap_res')
    
          if(sap_res && sap_res.STATUS == '1'){ 
    
            toast.success('FCI Vendor Assignment Process Sent to SAP Successfully!')    
            ...
            setFetch(true)
            Swal.fire({
              title: 'Sent FCI VA Info to SAP process Failed in SAP. Kindly contact admin..!',
              icon: "warning",
      ...
      else if (tripInfo.vehicle_type_id.id != '4' && (tripInfo.trip_sheet_info.sap_flag == '1' || tripInfo.trip_sheet_info.sap_flag == '2')) {
        ...
        SAPData.append('TRIP_SHEET', tripInfo.trip_sheet_info.trip_sheet_no)
        SAPData.append('VEHICLE_NO', tripInfo.vehicle_number)
        SAPData.append('VEHICLE_TYPE', vehicle_type_find(tripInfo.vehicle_type_id.id))
        ...
        // SAPData.append('Purpose', '')
        // SAPData.append('Division', '')
        SAPData.append('Purpose', trip_info_find('purpose'))
        SAPData.append('Division', tripInfo.trip_sheet_info.purpose == '4' ? '' : trip_info_find('division'))
        SAPData.append('Stop_Flag', '4')

        // TripSheetInfoService.StopTSInfoToPP(SAPData).then((response) => {
        TripSheetInfoService.StopTSInfoToSAP(SAPData).then((response) => {
          ...
          if (response.data[0].STATUS == '1') {
            ...
            TripSheetClosureService.createTripsheetSettlement(formData)
              .then((res) => {
      ...
      /* ===== Own Vehicle Toll Amount Posting Process Start ===== */
        ...
        SAPApendData.TRIP_SHEET = tripInfo.trip_sheet_info.trip_sheet_no
        ...
        TripSheetClosureSapService.tollExpensePost(SAPApendData_Seq).then((res) => {
      ...
      setFetch(true)
      TripSheetClosureService.updateTripsheetSettlement(tripSettlementID, formData)
        .then((res) => {
          console.log(res)
          if (res.status == 200) {
            setFetch(true)
          ...
          {screenAccess && restrictScreenById ? (
            <>
                ...
                <CTabContent className="p-3">
                  <CTabPane role="tabpanel" aria-labelledby="home-tab" visible={mainKey === 1}>
                    ...
                    <CNav variant="tabs" role="tablist">
                      ...
                      { (tripInfo && tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.purpose == '2' && stoTableDataFGSTO.length == 0) && (
                        <>
                          <CNavItem>
                            <CNavLink
                              active={activeKey === 4}
                      ...
                      {sto_enable && tripInfo && tripInfo.trip_sheet_info.purpose == '3' && (
                        <>
                          <CNavItem>
                            <CNavLink
                              active={activeKey === 8}
                      ...
                      {sto_enable && tripInfo && tripInfo.trip_sheet_info.purpose == '4' && (
                        <>
                          <CNavItem>
                            <CNavLink
                              active={activeKey === 8}
              ...
              <CTabContent>
                <CTabPane role="tabpanel" aria-labelledby="home-tab" visible={activeKey === 1}>
                  ...
                  <CRow className="mt-2">
                    ...
                    <CCol xs={12} md={3}>
                      ...
                      {/* <CFormInput size="sm" id="diffmil" value={tripInfo.trip_sheet_info.purpose} readOnly /> */}
                    </CCol>
                  </CRow>
                  {tripInfo && tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.purpose == 4 && (
                    <>
                ...
                <CTabPane role="tabpanel" aria-labelledby="profile-tab" visible={activeKey === 7}>
                  ...
                  {incotermFreightInfo.length > 0 && tripInfo.trip_sheet_info.purpose == 1 && tripInfo.trip_sheet_info.to_divison == 1 &&
                    (
                      <>
                                  ...
                                  <>
                                    <CTableRow style={{ width: '100%' }}>
                                      ...
                                      <CTableHeaderCell
                                        ...
                                        style={{ width: '5%', textAlign: 'center' }}
                                      >
                                        {JavascriptInArrayComponent(data.inco_term_id,[381,382]) ? 0 : tripInfo.trip_sheet_info.freight_rate_per_tone}
                                      </CTableHeaderCell>
                                      ...
                                      >
                                        {/* {data.amount} */} 
                                        {Math.round(Number(parseFloat(Number(tripInfo.trip_sheet_info.freight_rate_per_tone)*Number(data.qty)).toFixed(2)))}
                                      </CTableHeaderCell>
                  ...
                  {!tripInfo.advance_payment_info && idt.length > 0 && tripInfo.trip_sheet_info.purpose == 1 && tripInfo.trip_sheet_info.to_divison == 1 &&
                  (
                      <>
                        <CRow className="mt-2" hidden>
                          <CCol xs={12} md={3}>
                  ...
                  <CTable caption="top" hover>
                    ...
                    <CTableBody>
                      <CTableRow>
                        ...
                        {stoTableDataFGSTO && stoTableDataFGSTO.length > 0 && (
                          <>
                            {stoTableDataFGSTO.map((data, index) => {
                              return (
                                <CTableDataCell key={`freight_load_tonnage_data_fgsto`}>
                                  <CFormInput
                                    size="sm"
                                    maxLength={5}
                                    id={`freight_qty_fgsto`}
                                    name={`freight_qty_fgsto`}
                                    value={hire_total_freight1(stoTableDataFGSTO)}
                                    readOnly
                                  />
                                </CTableDataCell>
                              )
                        ...
                        {stoTableDataRMSTO && stoTableDataRMSTO.length > 0 && (
                          <>
                            <CTableDataCell key={`freight_load_tonnage_data`}>
                              <CFormInput
                                size="sm"
                                maxLength={5}
                                id={`freight_qty`}
                                name={`freight_qty`}
                                value={hire_total_freight(stoTableDataRMSTO)}
                                readOnly
                              />
                            </CTableDataCell>
                          </>
                          ...
                          <>
                            <CTableDataCell key={`freight_load_tonnage_data`}>
                              <CFormInput
                                ...
                                id={`freight_qty`}
                                name={`freight_qty`}
                                value={others_hire_total_qty(stoOthersTableData)}
                                readOnly
                              />
                            </CTableDataCell>
                          </>
                        )}
                        {/* Freight : Load Tonnage in MTS Part End */}

                        {/* Freight : Total Freight Amount Part Start */}
                        {/* {shipmentInfo && shipmentInfo.length > 0 && (
                          <>
                            {shipmentInfo.map((data, index) => {
                              return (
                                <CTableDataCell key={`freight_total_amount_data${index}`}>
                                  <CFormInput
                                    size="sm"
                                    id={`freight_total_amount_${index}`}
                                    maxLength={5}
                                    name={`freight_total_amount_${index}`}
                                    value={data.shipment_freight_amount}
                                    readOnly
                                  />
                                </CTableDataCell>
                              )
                        ...
                        {stoTableData && stoTableData.length > 0 && (
                          <>
                            {stoTableData.map((data, index) => {
                              // setFreight_total_amount(data.sto_freight_amount)
                              return (
                                <CTableDataCell key={`freight_total_amount_data${index}`}>
                                  <CFormInput
                                    size="sm"
                              ...
                              return (
                                <CTableDataCell key={`freight_total_amount_data${index}`}>
                                  <CFormInput
                                    ...
                                    id={`freight_total_amount_${index}`}
                                    name={`freight_total_amount_${index}`}
                                    value={data.sto_freight_amount_rmsto}
                                    readOnly
                                  />
                                </CTableDataCell>
                              )
                        ...
                        <CTableDataCell key={`low_tonnage_amount_data`}>
                          <CFormInput
                            ...
                            value={
                              tripInfo.advance_payment_info
                                ? (Number(tripInfo.advance_payment_info.low_tonnage_charges) > 0 ? tripInfo.advance_payment_info.low_tonnage_charges : 0)
                                : 0
                            }
                            readOnly
                          />
                    ...
                    <CTableBody>
                      ...
                      {/* ================== Weighment Charges Part Start ======================= */}
                       <CTableRow>
                        ...
                        <CTableDataCell>
                          <CFormInput
                            className={tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.to_divison == 2 ? '' : `${errors.weighment_charges && 'is-invalid'}`}
                            id="weighment_charges"
                            name="weighment_charges"
                            maxLength={5}
                            onFocus={tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.to_divison == 2 ? undefined: onFocus}
                            onBlur={tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.to_divison == 2 ? undefined : onBlur}
                            readOnly={tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.to_divison == 2 ? true : false}
                            // onChange={handleChange} 
                            onChange={tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.to_divison == 2 ? '': handleChange}
                            required={tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.to_divison == 2 ? false : (errors.weighment_charges ? true : false)}
                            size="sm"
                            value={values.weighment_charges}
                          />
                          {tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.to_divison != 2 && errors.weighment_charges && (
                      ...
                      <CTableRow>
                        ...
                        <CTableDataCell>
                          <CFormInput
                            className={tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.to_divison == 2 ? '' : `${errors.diversion_return_charges && 'is-invalid'}`}
                            ...
                            readOnly={tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.to_divison == 2 ? true : false}
                            onChange={tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.to_divison == 2 ? '': handleChange}
                            required={tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.to_divison == 2 ? false : (errors.diversion_return_charges ? true : false)}
                            size="sm"
                            value={values.diversion_return_charges}
                          />
                          {tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.to_divison != 2 && errors.diversion_return_charges && (
                            <span className="small text-danger">
            ...
            <CTabPane role="tabpanel" aria-labelledby="profile-tab" visible={mainKey === 2}>
              ...
              <CNav variant="tabs" role="tablist">
                ...
                { (fgstoAvailable || (tripInfo && tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.purpose == '2' && stoTableDataFGSTO.length == 0)) && (
                  <>
                    <CNavItem>
                      <CNavLink
                        style={{
                ...
                {((sto_enable && tripInfo && tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.purpose == '3') || rmstoAvailable) && (
                  <>
                    <CNavItem>
                      <CNavLink
                        style={{
                ...
                { (othersAvailable || (tripInfo && tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.purpose == '4' && stoOthersTableData.length == 0)) && (
                  <>
                    <CNavItem>
                      <CNavLink
                        style={{
                ...
                {(( tripInfo && tripInfo.trip_sheet_info && tripInfo.trip_sheet_info.purpose == '5') || fciAvailable) && (
                  <>
                    <CNavItem>
                      <CNavLink
                        style={{
                ...
                <CTabPane role="tabpanel" aria-labelledby="contact-tab" visible={activeKey_2 === 3}>
                  ...
                  >
                    ...
                    <CTableBody>
                      ...
                      {tripInfo && stoTableDataFCI && stoTableDataFCI.length > 0 && (
                        <>
...
export default TripSheetClossure
