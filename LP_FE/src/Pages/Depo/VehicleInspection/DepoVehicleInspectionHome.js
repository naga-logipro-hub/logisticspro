...
const DepoVehicleInspectionHome = () => {
  /*================== User Location Fetch ======================*/
  ...
  const loadVehicleInspectionTable = () => {
    DepoVInspectionService.getVehicleReadyToInspect().then((res) => {
      setFetch(true)
        ...
        rowDataList.push({
          ...
          Action: (
            <CButton className="badge" color="warning">
              <Link className="text-white" to={`vehicleInspection/${data.depo_parking_yard_gate_id}`}>
                Vehicle Inspection
              </Link>
            </CButton>
          ),
...
export default DepoVehicleInspectionHome
