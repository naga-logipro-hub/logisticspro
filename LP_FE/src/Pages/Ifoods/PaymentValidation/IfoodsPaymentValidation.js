...
const IfoodsPaymentValidation = () => {
  ...
  const deleteTripAllPayment = () => {
    ...
    IfoodsExpenseClosureService.rejectAllTripPayment(formDataUpdate).then((res) => {
      console.log(res)
      setFetch(true)
      if (res.status == 200) {
        Swal.fire({
          ...
          navigation('/IfoodsPaymentValidationTable')
        });
  ...
  useEffect(() => {
    ...
    //        Shipment_Freight: data.shipment_info.freight_type == '2' ? data.shipment_info.shipment_depo_actual_freight_amount : data.shipment_info.shipment_depo_budget_freight_amount,
            ...
            // ),
            Tripsheet_No: (
              <Link className='text-black' target='_blank' to={`/IfoodsPaymentValidationTable/IfoodsPaymentValidationChild/${data.closure_info.closure_id}`}>
  ...
  const validationApproval = () => {
    ...
    form_data.append('scm_remarks', values.apremarks)
    form_data.append('reference_text', reference)
    form_data.append('status', 8)
    // form_data.append('invoice_posting_date', invoicePostingDate)
    form_data.append('invoice_copy', invoiceCopy)
    IfoodsExpenseClosureService.sendValidationApproval(form_data).then((res)=>{
      ...
      if (res.status == 200) {
        Swal.fire({
          ...
          icon: "success",
          text: 'Approval Request Sent to - Operational Head',
          confirmButtonText: "OK",
        }).then(function () {
          navigation('/IfoodsPaymentValidationTable')
        });
...
{/* 
                ...
                <CRow className="m-2">
                  <CCol xs={12} md={3}>
                    ...
                    <CFormLabel htmlFor="apremarks"> SCM - Remarks </CFormLabel>
                    <CFormTextarea
                      name="apremarks"
                      id="apremarks"
                      rows="1"
                ...
                <CRow className="mt-2">
                  <CCol className="m-2" xs={12} sm={12} md={3}>
                    <CButton size="sm" color="primary" className="text-white" type="button">
                      <Link className="text-white" to="/IfoodsPaymentValidationTable">
                        Previous
                      </Link>
                    </CButton>
                  </CCol>
...
export default IfoodsPaymentValidation
