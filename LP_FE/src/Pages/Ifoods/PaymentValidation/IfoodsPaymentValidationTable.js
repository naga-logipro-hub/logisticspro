...
const IfoodsPaymentValidationTable = () => {
  /*================== User Location Fetch ======================*/
  ...
  const getClosureVehiclesData = () => {
    IfoodsExpenseClosureService.getTripsReadyToSettlementValidation().then((res) => {
         ...
         // Contractor_Location: getLocationNameByCode(data.vendor_info.contractor_location),
          ...
          Action: (
            ...
            <CButton className="btn btn-success btn-sm me-md-1">
              <Link className="text-white" to={`IfoodsPaymentValidation/${data.id}`}>
                <i className="fa fa-eye" aria-hidden="true"></i>
              </Link>
            </CButton>
          ),
...
export default IfoodsPaymentValidationTable
