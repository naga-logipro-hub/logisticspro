    ...
    IfoodsVendorMasterService.getIfoodsVendors().then((response) => {
      ...
      viewData.map((data, index) => {
        rowDataList.push({
          ...
          Status: data.vendor_status === 1 ? '✔️' : '❌',
          Action: (
            <div className="d-flex justify-content-space-between">
              <CButton
                size="sm"
                color={data.vendor_status === 1 ? 'success' : 'danger'}
                shape="rounded"
                id={data.id}
                onClick={() => {
              ...
              <Link to={data.vendor_status === 1 ? `IfoodsVendorMaster/${data.id}` : ''}>
                <CButton
                  disabled={data.vendor_status === 1 ? false : true}
                  size="sm"
                  color={data.vendor_status === 1 ? 'success' : 'secondary'}
                  shape="rounded"
                  id={data.id}
                  className="m-1"
                  type="button"
          ...
          {screenAccess ? (
            <>
              <CRow className="mt-1 mb-1">
                ...
                >
                  <Link className="text-white" to="/IfoodsVendorMaster">
...
export default IfoodsVendorMasterTable
