...
const IfoodsPaymentValidationOhTable = () => {
  /*================== User Location Fetch ======================*/
  ...
  const getClosureVehiclesData = () => {
    IfoodsExpenseClosureService.getTripsReadyToSettlementValidationOh().then((res) => {
      ...
      let rowDataList = []
          ...
          Vendor_Name: data.vendor_info.vendor_name,
          Request_By: data.payment_user_info.emp_name,
          Trip_Count:data.trip_info.length,
         // Contractor_Location: getLocationNameByCode(data.vendor_info.contractor_location),
          Waiting_At: (
            <span className="badge rounded-pill bg-info">
              {data.status == '1'
                ? 'Payment Submission Completed'
                : 'SCM - Validation Completed'}
            </span>
          ),
          ...
          Action: (
            ...
            <CButton className="btn btn-success btn-sm me-md-1">
              <Link className="text-white" to={`IfoodsPaymentValidationOh/${data.id}`}>
                <i className="fa fa-eye" aria-hidden="true"></i>
              </Link>
            </CButton>
          ),
...
export default IfoodsPaymentValidationOhTable
