...
const ShipmentCreationNLCDReport = () => {
  ...
  const exportToCSV = () => {
    ...
    let fileName='NLCD_Shipment_Report_'+dateTimeString
  ...
  const loadTripShipmentReport = (fresh_type = '') => {
    ...
    if (fresh_type !== '1') {
      ...
      VehicleAssignmentService.getShipmentDataForReportNLCD().then((res) => {
        tableReportData = res.data.data
        console.log(tableReportData)

        let rowDataList = []
        let filterData = tableReportData.filter(
          (data) => user_locations.indexOf(data.vehicle_location_id) != -1 && data.assigned_by == 2
        )
        ...
        filterData.map((data, index) => {
          rowDataList.push({
            ...
            Vehicle_Type: vehicleType(data.vehicle_type_id),
            ...
            Action: (
              <CButton
                ...
                color="warning"
                id={data.id}
                // disabled={data.shipment_status != 5}
                size="sm"
              >
          ...
          rowDataList.push({
            ...
            Action: (
              <CButton
                ...
                color="warning"
                id={data.id}
                // disabled={data.shipment_statusstatus != 5}
                size="sm"
              >
                <span className="float-start">
                <Link to={`${data.shipment_id}||${data.parking_id}`}>
...
export default ShipmentCreationNLCDReport
